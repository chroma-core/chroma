...
def step_server_config(context, server_fqdn: str, server_port: str):
    ...
    if "N_GPU_LAYERS" in os.environ:
        ...
        print(
            f"$N_GPU_LAYERS set, overriding n_gpu_layer with to {context.n_gpu_layer}"
        )
...
def step_start_server(context):
    ...
    addrs = socket.getaddrinfo(
        context.server_fqdn, context.server_port, type=socket.SOCK_STREAM
    )
...
async def step_wait_for_the_server_to_be_started(
    context, expecting_status: Literal["healthy", "ready", "idle", "busy"] | str
):
    match expecting_status:
        case "healthy":
            await wait_for_slots_status(context, context.base_url, 200, timeout=30)

        case "ready" | "idle":
            await wait_for_slots_status(
                context,
                context.base_url,
                200,
                timeout=30,
...
@async_run_until_complete
async def step_all_slots_status(
    context, expected_slot_status_string: Literal["idle", "busy"] | str
):
    ...
    expected_slots = [
        {"id": slot_id, "state": expected_slot_status}
...
def step_impl(context, n_prompt):
    assert (
        n_prompt < 0 or n_prompt == context.completion["timings"]["prompt_n"]
    ), f"n_prompt={context.completion['timings']['prompt_n']}"
...
@step("{n_ga:d} group attention factor" " to extend context size through self-extend")
...
def step_fixed_prompts(context, n_prompts):
    context.prompts.extend(
        [
            str(0) * (context.n_batch if context.n_batch is not None else 512)
...
@step(
    'a "{passkey}" passkey challenge prompt with the passkey inserted every {i_pos:d} junk'
)
...
async def step_oai_chat_completions(context, api_error):
    ...
    expect_api_error = api_error == "raised"
    seeds = (await completions_seed(context, num_seeds=1),)
    completion = await oai_chat_completions(
        context.prompts.pop(),
        seeds[0] if seeds is not None else seeds,
        ...
        if hasattr(context, "response_format")
        else None,
...
async def all_prompts_are_predicted(context, expected_predicted_n=None):
    ...
    for i in range(n_completions):
        assert_n_tokens_predicted(
            context.tasks_result.pop(), expected_predicted_n=expected_predicted_n
        )
    assert (
        len(context.concurrent_tasks) == 0
    ), f"{len(context.concurrent_tasks)} pending requests"
...
async def step_compute_embedding(context):
    context.n_prompts = 1
    context.embeddings = await request_embedding(
        context_text(context), None, base_url=context.base_url
    )
    ...
    for i in range(n - 1):
        for j in range(i + 1, n):
            ...
            if context.debug:
                print(f"{msg}")
            assert np.isclose(
                similarity, 1.0, rtol=1e-05, atol=1e-08, equal_nan=False
            ), msg


@step("embeddings are generated")
def step_assert_embeddings(context):
    assert context.n_prompts == len(context.embeddings), (
...
async def step_oai_compute_embeddings(context):
    context.n_prompts = 1
    context.embeddings = await request_oai_embeddings(
...
async def step_options_request(context, origin):
    async with aiohttp.ClientSession() as session:
        ...
        async with session.options(
            f"{context.base_url}/v1/chat/completions", headers=headers
        ) as response:
            assert response.status == 200
...
async def step_prometheus_metrics_exported(context):
    async with aiohttp.ClientSession() as session:
        async with await session.get(f"{context.base_url}/metrics") as metrics_response:
            ...
            assert (
                metrics_response.headers["Content-Type"] == "text/plain; version=0.0.4"
            )
            ...
            assert (
                int(metrics_response.headers["Process-Start-Time-Unix"]) > 0
            ), "no header process start time"
...
def step_assert_metric_value(context, metric_name, metric_value):
    ...
    assert (
        context.metrics[metric_name].samples[0].value == metric_value
    ), f"metric: {context.metrics[metric_name]}"
...
def step_available_models(context):
    ...
    openai.api_key = (
        context.user_api_key if context.user_api_key is not None else "nope"
    )
...
def step_supported_models(
    context,
    i_model: int,
    param: Literal["identified", "trained"] | str,
    preposition: str,
    param_value: str,
):
...
async def concurrent_requests(context, f_completion, *args, **kwargs):
    ...
    for prompt_no in range(context.n_prompts):
        ...
        context.concurrent_tasks.append(
            asyncio.create_task(f_completion(*shifted_args, **kwargs))
        )
...
async def step_save_slot(context, slot_id, filename):
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"{context.base_url}/slots/{slot_id}?action=save",
...
async def step_restore_slot(context, slot_id, filename):
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"{context.base_url}/slots/{slot_id}?action=restore",
            json={"filename": filename},
            headers={"Content-Type": "application/json"},
        ) as response:
            context.response = response
...
async def step_erase_slot(context, slot_id):
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"{context.base_url}/slots/{slot_id}?action=erase",
            headers={"Content-Type": "application/json"},
        ) as response:
            context.response = response
...
async def toggle_lora_adapter(context, on_or_off: str, lora_id: int):
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"{context.base_url}/lora-adapters",
...
async def oai_chat_completions(
    ...
    completion_response = {"content": "", "timings": {"predicted_n": 0, "prompt_n": 0}}
    if async_client:
        origin = "llama.cpp"
        headers = {"Authorization": f"Bearer {user_api_key}", "Origin": origin}
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{base_url}{base_path}", json=payload, headers=headers
            ) as response:
                if enable_streaming:
                    assert response.status == 200
                    assert response.headers["Access-Control-Allow-Origin"] == origin
                    ...
                    while event_received:
                        ...
                        async for line_in_bytes in response.content:
                            ...
                            assert (
                                event_data[0] == "data"
                            ), f"Bad event code received: ```{event_data}```"
                            chunk_raw = event_data[1]

                            chunk = json.loads(chunk_raw)
                            assert (
                                len(chunk["choices"]) == 1
                            ), f"no choices provided, line ```{line}```"
                    ...
                    if expect_api_error is None or not expect_api_error:
                        ...
                        completion_response = {
                            ...
                            "timings": {
                                "predicted_n": chat_completion_raw["usage"][
                                    "completion_tokens"
                                ],
                                "prompt_n": chat_completion_raw["usage"][
                                    "prompt_tokens"
                                ],
                            },
                        }
...
async def request_oai_embeddings(
    input, seed, base_url=None, user_api_key=None, model=None, async_client=False
) -> list[list[float]]:
    # openai client always expects an api_key
    user_api_key = user_api_key if user_api_key is not None else "nope"
    if async_client:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                assert (
                    response.status == 200
                ), f"received status code not expected: {response.status}"
                assert response.headers["Access-Control-Allow-Origin"] == origin
                assert (
                    response.headers["Content-Type"]
                    == "application/json; charset=utf-8"
                ...
                ), f"invalid model received: {response_json['model']}"
...
def assert_n_tokens_predicted(
    completion_response, expected_predicted_n=None, re_content=None
):
    ...
    if re_content is not None:
        p = re.compile(
            re_content,
            flags=RegexFlag.IGNORECASE | RegexFlag.MULTILINE | RegexFlag.DOTALL,
        )
    ...
    if expected_predicted_n and expected_predicted_n > 0:
        assert n_predicted == expected_predicted_n, (
            f"invalid number of tokens predicted:"
...
async def wait_for_slots_status(
    context,
    base_url,
    ...
    if context.debug:
        print(
            f"Starting checking for health for expected_http_status_code={expected_http_status_code}"
        )
    interval = 0.5
    counter = 0
    if "GITHUB_ACTIONS" in os.environ:
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            async with await session.get(
                f"{base_url}/slots", params=params
            ) as slots_response:
                ...
                if status_code == 200 and status_code == expected_http_status_code:
                    ...
                    n_slots_processing = sum(
                        1 if slot["state"] != 0 else 0 for slot in slots
                    )
                    if (slots_idle is None or slots_idle == n_slots_idle) and (
                        slots_processing is None
                        or slots_processing == n_slots_processing
                    ):
                        return
            ...
            if counter >= timeout:
                ...
                if expected_http_status_code == 503:
                    if len(context.tasks_result) == 0:
                        print(
                            "\x1b[5;37;43mWARNING: forcing concurrent tasks,"
...
def assert_slots_status(slots, expected_slots):
    ...
    for slot_id, (expected, slot) in enumerate(zip(expected_slots, slots)):
        for key in expected:
            assert expected[key] == slot[key], (
                f"invalid slot {slot_id}"
...
async def completions_seed(context, num_seeds=None):
    if hasattr(context, "seed") and context.seed is not None:
        ...
        context.seed = (
            context.seed[num_seeds:] if num_seeds < context.n_prompts else None
        )
        return seeds
...
def start_server_background(context):
    ...
    context.server_process = subprocess.Popen(
        [str(arg) for arg in [context.server_path, *server_args]], **pkwargs
    ...
    thread_stdout = threading.Thread(
        target=server_log, args=(context.server_process.stdout, sys.stdout)
    )
    thread_stdout.start()

    thread_stderr = threading.Thread(
        target=server_log, args=(context.server_process.stderr, sys.stderr)
    )
    thread_stderr.start()
