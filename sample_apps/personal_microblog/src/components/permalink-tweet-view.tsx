...
export default function PermalinkTweetView({ post, aiReplies, existingReplies, relatedPosts }: { post: TweetModel, aiReplies: TweetModel[], existingReplies: TweetModel[], relatedPosts: TweetModel[] }) {
        ...
        <div className="pt-4 pb-6">
          <div className="bg-[#fafafa] p-5">
            <div className="mb-4">
              ...
              {aiReplies.length > 0 && (
                <div className="font-ui text-gray-500 mt-2">
                  <MarkdownContent content={aiReplies.map((r) => r.body).join("\n")} className="font-ui text-sm"/>
                </div>
              )}
            </div>
            {replies.map((r) => (
              <div key={r.id} className="p-4 border-l border-[.5px]">
                <MarkdownContent content={r.body} className="font-body"/>
              </div>
            ))}
          </div>
