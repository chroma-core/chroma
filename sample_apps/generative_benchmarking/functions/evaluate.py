...
def query_collection(
    ...
    for i in tqdm(
        range(0, len(query_embeddings), BATCH_SIZE), desc="Processing batches"
    ):
        ...
        for idx, (query_id, query_embedding) in enumerate(
            zip(batch_ids, batch_embeddings)
        ):
    ...
    return QueryResults(doc_scores=results)


def get_metrics(
    qrels: QueryRelevance, results: Dict[str, Dict[str, float]], k_values: List[int]
) -> ResultMetrics:
    recall = dict()
    precision = dict()
    map = dict()
    ...
    evaluator = pytrec_eval.RelevanceEvaluator(
        qrels_relevances, {map_string, ndcg_string, recall_string, precision_string}
    )
    ...
    return ResultMetrics(ndcg=ndcg, map=map, recall=recall, precision=precision)


def evaluate(
    k_values: List[int], qrels_df: pd.DataFrame, results_dict: QueryResults
) -> ResultMetricsDict:
    qrels = (
        qrels_df.groupby("query-id")
        .apply(lambda g: dict(zip(g["corpus-id"], g["score"])))
        .to_dict()
    )
    ...
    for query_id, query_data in results_dict.doc_scores.items():
        ...
        for doc_id, score in zip(
            query_data.retrieved_corpus_ids, query_data.all_scores
        ):
            results[query_id][doc_id] = score

    result_metrics = get_metrics(
        qrels=qrels_relevances, results=results, k_values=k_values
    )
    ...
    return ResultMetricsDict(results=final_result)
...
def run_benchmark(
    ...
    result_metrics = evaluate(
        k_values=k_values, qrels_df=qrels, results_dict=query_results_scores
    )
