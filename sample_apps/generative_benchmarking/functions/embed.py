...
def minilm_embed_in_batches(
    model: Any, texts: List[str], batch_size: int = 100
) -> List[List[float]]:
    all_embeddings = []
...
def openai_embed(
    openai_client: OpenAIClient, texts: List[str], model: str
) -> List[List[float]]:
    try:
        return [
            response.embedding
            for response in openai_client.embeddings.create(
...
def openai_embed_in_batches(
    openai_client: OpenAIClient, texts: List[str], model: str, batch_size: int = 100
) -> List[List[float]]:
    all_embeddings = []
...
def jina_embed(
    JINA_API_KEY: str, input_type: str, texts: List[str]
) -> List[List[float]]:
    try:
...
def jina_embed_in_batches(
    JINA_API_KEY: str, input_type: str, texts: List[str], batch_size: int = 100
) -> List[List[float]]:
    all_embeddings = []
...
def voyage_embed(
    voyage_client: VoyageClient, input_type: str, texts: List[str]
) -> List[List[float]]:
    try:
