//! Token loading: read tokens captured by `notion-internal-dump login`.
//!
//! Resolution order (matches what `cmd::login` writes; we walk up from
//! CWD looking for a directory containing `notion_cli/` so users can
...
//! 5. Fail with a pointer to `notion-internal-dump login`
//!
...
pub fn load(token_v2_arg: Option<&str>) -> Result<Tokens> {
    ...
    Err(anyhow!(
        "no token_v2 found. Run `notion-internal-dump login` to obtain a \
         session, or pass --token-v2 / set NOTION_TOKEN_V2."
    ))
}
