...
const FilterSidebar: React.FC<FilterSidebarProps> = ({ showSkeleton }) => {
  ...
  const [visibleDatapoints, setvisibleDatapoints] = useAtom(visibleDatapointsAtom)
    ...
    <Flex
      ...
      {!showSkeleton ?
        <Flex key="buttons" px={3} justifyContent="space-between" alignContent="center">
          ...
          <Text fontSize="sm">{visibleDatapoints.length} / {Object.keys(datapoints).length} total</Text>
        </Flex>
        : null}
      <Divider w="100%" pt={2} />
