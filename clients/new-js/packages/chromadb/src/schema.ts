...
const buildEmbeddingFunctionFromConfig = <T extends AnyEmbeddingFunction>(
  config: EmbeddingFunctionConfiguration,
  registry: Map<string, { buildFromConfig(config: Record<string, any>): T }>,
  fallback: (() => T | undefined) | undefined = undefined,
): T | undefined => {
  if (!config || config.type === "legacy") {
    return fallback ? fallback() : undefined;
  }

  const builder = registry.get(config.name);
  if (!builder) {
    console.warn(
      `Embedding function '${config.name}' is not registered. Returning undefined from schema deserialization.`,
    );
    return fallback ? fallback() : undefined;
  }

  try {
    return builder.buildFromConfig(config.config as Record<string, any>);
  } catch (err) {
    console.warn(
      `Embedding function '${config.name}' failed to build from configuration. Error: ${err}`,
    );
    return fallback ? fallback() : undefined;
  }
};
...
export class Schema {
  ...
  private static deserializeVectorConfig(json: Record<string, any>): VectorIndexConfig {
    ...
    const embeddingFunction = buildEmbeddingFunctionFromConfig<EmbeddingFunction>(
  ...
  private static deserializeSparseVectorConfig(json: Record<string, any>): SparseVectorIndexConfig {
    ...
    const embeddingFunction = buildEmbeddingFunctionFromConfig<SparseEmbeddingFunction>(
      json.embedding_function as EmbeddingFunctionConfiguration,
      knownSparseEmbeddingFunctions,
      () => (config.embeddingFunction as SparseEmbeddingFunction | null | undefined) ?? undefined,
    );
