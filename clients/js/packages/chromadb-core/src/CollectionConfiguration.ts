...
export function validateCreateHnswConfig(
  ...
  const currentCpuCount = os.cpus().length;
  ...
  if (config.num_threads !== undefined) {
    if (config.num_threads > currentCpuCount) {
      throw new InvalidConfigurationError(
        `num_threads (${config.num_threads}) must be less than or equal to the number of available processors (${currentCpuCount})`,
      );
    }
