...
import {
  ...
  InputType,
} from "../../src/embeddings/VoyageAIEmbeddingFunction";
...
if (!process.env.VOYAGE_API_KEY) {
  ...
  test("it should add VoyageAI embeddings", async () => {
    ...
    const embedder = new VoyageAIEmbeddingFunction({
      voyageaiApiKey: process.env.VOYAGE_API_KEY || "",
      modelName: "voyage-2",
      batchSize: 5,
  ...
  test("it should throw an exception when the batch size is smaller than the number of texts", async () => {
    ...
    try {
      ...
      expect(e.message).toBe(
        "Error calling VoyageAI API: The number of texts to embed exceeds the maximum batch size of 2"
      );
    }
  });
}
