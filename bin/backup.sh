...
docker exec -u 0 -it chroma-clickhouse-1 clickhouse-client --query="BACKUP DATABASE default TO Disk('backups', '$backup_name.zip')"

# use that name to dump the clickhouse db and copy into the folder
docker cp chroma-clickhouse-1:/etc/clickhouse-server/$backup_name.zip ../backups/$backup_name/$backup_name.zip

# remove the backup from teh clickhouse container
docker exec -u 0 -it chroma-clickhouse-1 rm /etc/clickhouse-server/$backup_name.zip

# copy the entire contents of default index directory
docker cp chroma-server-1:/index_data ../backups/$backup_name
