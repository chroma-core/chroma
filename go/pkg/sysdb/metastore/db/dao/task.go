...
func (s *taskDb) AdvanceTask(taskID uuid.UUID, taskRunNonce uuid.UUID, completionOffset int64, nextRunDelaySecs uint64) (*dbmodel.AdvanceTask, error) {
	...
	now := time.Now()
	// Bump next_nonce to mark a new epoch, but don't touch lowest_live_nonce yet
	// lowest_live_nonce will be updated later by finish_task when verification completes
	result := s.db.Model(&dbmodel.Task{}).Where("task_id = ?", taskID).Where("is_deleted = false").Where("next_nonce = ?", taskRunNonce).UpdateColumns(map[string]interface{}{
	...
	var task dbmodel.Task
	if err := result.Scan(&task).Error; err != nil {
		log.Error("AdvanceTask: failed to scan result", zap.Error(err), zap.String("task_id", taskID.String()))
		return nil, err
	}
...
func (s *taskDb) UpdateCompletionOffset(taskID uuid.UUID, taskRunNonce uuid.UUID, completionOffset int64) error {
	...
	// Validate that we're updating the correct epoch by checking lowest_live_nonce = taskRunNonce
	// This ensures we're updating the completion offset for the exact epoch we're working on
	result := s.db.Model(&dbmodel.Task{}).
		Where("task_id = ?", taskID).
		Where("is_deleted = false").
		Where("lowest_live_nonce = ?", taskRunNonce). // Ensure we're updating the correct epoch
	...
	if result.RowsAffected == 0 {
		log.Error("UpdateCompletionOffset: no rows affected - task not found or wrong epoch", zap.String("task_id", taskID.String()), zap.String("task_run_nonce", taskRunNonce.String()))
		return common.ErrTaskNotFound
	}
...
// FinishTask updates lowest_live_nonce to mark the current epoch as verified
...
func (s *taskDb) FinishTask(taskID uuid.UUID) error {
	now := time.Now()
	// Set lowest_live_nonce = next_nonce to indicate this epoch is fully verified
