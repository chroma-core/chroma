...
class FastAPI(BaseHTTPClient, ServerAPI):
    ...
    def get_pre_flight_checks(self) -> None:
        resp_json = self._make_request("get", "/pre-flight-checks")
        self.pre_flight_checks = resp_json

    @trace_method("FastAPI.get_base64_encoding", OpenTelemetryGranularity.OPERATION)
    def get_base64_encoding(self) -> bool:
        if self.pre_flight_checks is None:
            self.get_pre_flight_checks()
