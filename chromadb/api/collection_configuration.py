...
def load_collection_configuration_from_json(
    ...
    if json_map.get("spann") is not None and json_map.get("hnsw") is not None:
    ...
    if json_map.get("hnsw") is not None:
        hnsw_config = cast(HNSWConfiguration, json_map["hnsw"])
    if json_map.get("spann") is not None:
        spann_config = cast(SpannConfiguration, json_map["spann"])
...
def populate_create_spann_defaults(
    ...
    if config.get("search_nprobe") is None:
        config["search_nprobe"] = 128
    if config.get("write_nprobe") is None:
        config["write_nprobe"] = 128
