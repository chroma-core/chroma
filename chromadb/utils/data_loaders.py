...
class ImageURLoader(DataLoader[List[Optional[Image]]]):
    def __init__(
        self,
    ...
    def _load_image(self, url: Optional[URI]) -> Optional[Image]:
        auth = httpx.DigestAuth(self._username, self._password)
        return (
            np.asarray(self._PILImage.open(BytesIO(httpx.get(url, auth=auth).content)))
            if url is not None
            else None
        )
