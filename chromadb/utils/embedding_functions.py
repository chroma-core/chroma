...
class ONNXMiniLM_L6_V2(EmbeddingFunction):
    ...
    def __init__(self):
        # Import dependencies on demand to mirror other embedding functions. This breaks typechecking, thus the ignores.
        try:
    ...
    def _download(self, url: str, fname: Path, chunk_size=1024):
        ...
        with open(fname, "wb") as file, self.tqdm(  # type: ignore
            desc=str(fname),
            total=total,
            unit="iB",
            unit_scale=True,
    ...
    def _init_model_and_tokenizer(self):
        if self.model == None and self.tokenizer == None:
            self.tokenizer = self.Tokenizer.from_file(  # type: ignore
            ...
            self.model = self.ort.InferenceSession(  # type: ignore
    ...
    def _download_model_if_not_exists(self):
        ...
        if not os.path.exists(self.DOWNLOAD_PATH / self.ARCHIVE_FILENAME):
            ...
            self._download(self.MODEL_DOWNLOAD_URL, self.DOWNLOAD_PATH / self.ARCHIVE_FILENAME)
