...
class CLIPTextEmbeddingFunction(EmbeddingFunction):
    ...
    def __call__(self, texts: Documents) -> Embeddings:
        if self.device == "cpu":
            print(
                "Ensure to install PyTorch with GPU support if your system is equipped with a GPU. Install it with `pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118` \n"
            )
