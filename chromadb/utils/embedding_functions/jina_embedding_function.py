...
import numpy as np
import warnings
from chromadb.utils.muvera import generate_gaussian_vectors, create_fdes
import importlib
import base64
import io
...
class JinaEmbeddingFunction(EmbeddingFunction[Embeddable]):
    ...
    def _convert_resp(self, resp: Any, is_query: bool = False) -> Embeddings:
        ...
        if self.return_multivector:
            ...
            k_sim = 5
            g_vecs = generate_gaussian_vectors(dims, k_sim)
            fdes = create_fdes(multi_embeddings, g_vecs, is_query=is_query)
