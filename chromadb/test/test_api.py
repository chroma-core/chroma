...
def test_index_params(api_fixture, request):
    ...
    collection = api.create_collection(
        name="test_index_params", index_params={"space": "cosine", "ef": 20, "M": 5}
    )
    collection.add(**records)
    items = collection.query(
        query_embeddings=[0.6, 1.12, 1.6],
    ...
    # ip
    api.reset()
    collection = api.create_collection(name="test_index_params", index_params={"space": "ip"})
    collection.add(**records)
    items = collection.query(
        query_embeddings=[0.6, 1.12, 1.6],
        n_results=1,
...
# # test updating index_params on a collection
@pytest.mark.parametrize("api_fixture", test_apis)
def test_update_index_params(api_fixture, request):
    api = request.getfixturevalue(api_fixture.__name__)

    api.reset()
    original_index_params = {"space": "cosine", "ef": 20, "M": 5}
    collection = api.create_collection(name="test_index_params", index_params=original_index_params)

    assert collection.index_params == original_index_params

    # update index_params
    # TODO: should modify merge in new params with old params?, or just update the one property?
    # i think merge is better
    new_index_params = {"space": "l2", "ef": 20, "M": 5}
    collection.modify(index_params=new_index_params)

    assert collection.index_params == new_index_params

    collection.add(**records)

    # can no longer update the index params, because we have data
    with pytest.raises(Exception):
        collection.modify(index_params=new_index_params)
    ...
    new2_index_params = {"space": "ip", "ef": 200, "M": 15}
    collection.modify(index_params=new2_index_params)

    assert collection.index_params == new2_index_params


# test loading from disk where index_params isnt set yet (migration case)
#

# test persisting and loading these indexes from disk
# @pytest.mark.parametrize("api_fixture", [local_persist_api])
# def test_persist_index_loading(api_fixture, request):
#     api = request.getfixturevalue("local_persist_api")
#     api.reset()
...
# test allow updating the index type but only if the index is empty

# test other index params, EF and M
