...
@pytest.fixture
def duckdb_db():
    return chromadb.db.impl.duckdb.DuckDB(Settings(duckdb_database=":memory:", migrations="none"))
...
def test_migrations(db_fixture, request):
    ...
    all_migrations = migrations.source_migrations(
        dir="chromadb/test/db/migrations", scope=db.migration_scope()
    )

    with db.tx() as cur:
        unapplied_migrations = migrations.validate(cur, all_migrations)
...
def test_tampered_migration(db_fixture, request):
    ...
    with db.tx() as cur:
        ...
        migrations.validate(cur, all_migrations)
        ...
        with pytest.raises(migrations.InconsistentVersionError):
            migrations.validate(cur, inconsistent_version_migrations)
        ...
        with pytest.raises(migrations.InconsistentHashError):
            migrations.validate(cur, inconsistent_hash_migrations)
