...
class LocalHnswSegment(VectorReader):
    ...
    def query_vectors(
        ...
        with self._opentelemtry_client.trace(
            "LocalHnswSegment.query_vectors",
            OpenTelemetryGranularity.ALL,
        ):
            ...
            if ids is not None:
                labels = {
                    self._id_to_label[id] for id in ids if id in self._id_to_label
                }
                if len(labels) < k:
                    k = len(labels)
    ...
    def _ensure_index(self, n: int, dim: int) -> None:
        ...
        with self._opentelemtry_client.trace(
            "LocalHnswSegment._ensure_index",
            OpenTelemetryGranularity.ALL,
        ):
            if not self._index:
