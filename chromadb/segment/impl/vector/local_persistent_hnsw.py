...
class PersistentLocalHnswSegment(LocalHnswSegment):
    ...
    def _init_index(self, dimensionality: int) -> None:
        ...
        ef_search = self._configuration.get_parameter_value("ef_search")
        num_threads = self._configuration.get_parameter_value("num_threads")
        batch_size = self._configuration.get_parameter_value("batch_size")
        resize_factor = self._configuration.get_parameter_value("resize_factor")
    ...
    def _apply_batch(self, batch: Batch) -> None:
        sync_threshold = self._configuration.get_parameter_value("sync_threshold")
        super()._apply_batch(batch)
        if self._num_log_records_since_last_persist >= sync_threshold:
            self._persist()
    ...
    def _write_records(self, records: Sequence[LogRecord]) -> None:
        ...
        with WriteRWLock(self._lock):
            for record in records:
                ...
                if self._num_log_records_since_last_batch >= batch_size:
