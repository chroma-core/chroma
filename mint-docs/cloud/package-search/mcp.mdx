---
title: Package Search MCP
description: Add ground truth code context to AI agents via MCP.
---
...
4. Connect to the Chroma MCP server to search code packages. In this example, we search for class definitions in the `numpy` package from PyPI.
...
4. Get a Gemini API key in [Google's AI Studio](https://aistudio.google.com/app/apikey)

5. Connect the Chroma MCP server with Gemini to enable AI-powered code searches. In this example, we ask Gemini to explain how the Fast Fourier Transform algorithm is implemented in `numpy`, using the Chroma MCP tools to search and analyze the code.
...
4. Add the Chroma MCP server to Claude Code with your Chroma API key:
...
4. Add the following to your `~/.codex/config.toml` file with your Chroma Cloud API key:
...
4. In Cursor's settings, search for "MCP" and add the following configuration with your Chroma Cloud API key:
...
4. In Windsurf's settings, search for "MCP" and add the following configuration with your Chroma Cloud API key:
...
4. Add the following to your `~/Library/Application Support/Claude/claude_desktop_config.json`:
...
4. Add the following to your `~/.config/opencode/opencode.json` file with your Chroma Cloud API key:
...
4. Install the `ollmcp` package:

```bash
pip install ollmcp
```

5. Create an `mcp_config.json` file with the following content and your Chroma Cloud API key:
...
6. Start an Ollama MCP session with the path to your `mcp_config.json` file and model of choice:
...
4. Connect to the Chroma MCP server to search code packages. In this example, we search for the Fast Fourier Transform function in the `numpy` package from PyPI using the `package_search_grep` tool.
...
4. Add this to your Roo Code MCP server configuration:
