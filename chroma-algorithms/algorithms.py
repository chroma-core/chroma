...
def activation_uncertainty(training_data: pd.DataFrame, inference_data: pd.DataFrame) -> np.ndarray:
    ...
    for class_name in inference_data['inference_class'].unique():
        ...
        training_data_class_activation = np.linalg.norm(training_data_class['embedding'].tolist(), axis=1)

        inference_data_class = inference_data[inference_data['inference_class'] == class_name]
        inference_data_class_activation = np.linalg.norm(inference_data_class['embedding'].tolist(), axis=1)
        ...
        inference_positions = np.searchsorted(training_data_class_activation, inference_data_class_activation)
        inference_parcentiles = training_percentiles[inference_positions]

        activation_uncertainty[inference_data['inference_class'] == class_name] = inference_parcentiles
    
    return activation_uncertainty

def boundary_uncertainty(training_data: pd.DataFrame, inference_data: pd.DataFrame, ann_index: Hnswlib, n_neighbors: Optional[int] = 100, model_space: Optional[str] = "default_scope") -> np.ndarray:
    ...
    neighbor_ids, distances = ann_index.get_nearest_neighbors(model_space=model_space, query=inference_data['embedding'].tolist(), k=n_neighbors, uuids=training_ids)
    neighbor_ids = np.array(neighbor_ids)

    flat_idxs = [training_id_to_idx[n_id] for n_id in neighbor_ids.reshape(-1)]
    neighbor_categories = training_data['label_class'].iloc[flat_idxs].to_numpy().reshape(neighbor_ids.shape)
...
def class_outliers(training_data: pd.DataFrame, inference_data: pd.DataFrame, ann_index: Hnswlib, n_neighbors: Optional[int] = 100, model_space: Optional[str] = "default_scope") -> Tuple[np.ndarray, np.ndarray]:
    ...
    for class_name in inference_data.inference_class.unique():
        ...
        t_neighbor_ids, t_neighbor_dists = ann_index.get_nearest_neighbors(model_space=model_space, query=inference_data_class['embedding'].tolist(), k=n_neighbors, uuids=training_data_class['uuid'].tolist()) 
        v_neighbor_ids, v_neighbor_dists = ann_index.get_nearest_neighbors(model_space=model_space, query=inference_data_class['embedding'].tolist(), k=n_neighbors, uuids=inference_data_class['uuid'].tolist())
        ...
        representative_outliers[inference_data['inference_class'] == class_name] = dist_diff_percentiles

        overall_median = np.median(np.concatenate((t_neighbor_dists, v_neighbor_dists), axis=1), axis=1)
        overall_median_percentiles = np.argsort(overall_median) / len(overall_median)

        difficult_outliers[inference_data['inference_class'] == class_name] = overall_median_percentiles

    return representative_outliers, difficult_outliers

def cluster_outliers(training_data: pd.DataFrame, inference_data: pd.DataFrame, min_cluster_size: Optional[int] = 500, min_samples: Optional[int] = 500, metric: Optional[str]='euclidean') -> Tuple[np.ndarray, np.ndarray]:
    ...
    training_clusterer = hdbscan.HDBSCAN(min_cluster_size=min_cluster_size, min_samples=min_samples, metric=metric)
    training_clusterer.fit(training_data['embedding'].tolist())

    # Cluster the inference data
    inference_clusterer = hdbscan.HDBSCAN(min_cluster_size=min_cluster_size, min_samples=min_samples, metric=metric)
    ...
    labels, t_probabilities = hdbscan.approximate_predict(training_clusterer, inference_data['embedding'].tolist())

    representative_outliers = t_probabilities - inference_clusterer.probabilities_
    representative_outlier_percentiles = np.argsort(representative_outliers) / len(representative_outliers)
...
def random_sample(inference_data: pd.DataFrame, n_samples: Optional[int] = 1000) -> np.ndarray:
    ...
    return np.random.choice(len(inference_data), n_samples, replace=False)
