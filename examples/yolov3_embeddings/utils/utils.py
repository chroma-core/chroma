...
def rescale_boxes(boxes, current_dim, original_shape):
    ...
    output = boxes.new(boxes.shape)
    output[:, 0] = ((boxes[:, 0] - pad_x // 2) / unpad_w) * orig_w
    output[:, 1] = ((boxes[:, 1] - pad_y // 2) / unpad_h) * orig_h
    output[:, 2] = ((boxes[:, 2] - pad_x // 2) / unpad_w) * orig_w
    output[:, 3] = ((boxes[:, 3] - pad_y // 2) / unpad_h) * orig_h
    return output
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None):
    ...
    output_indices = [torch.zeros((0, 0), device="cpu")] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference

        # Create index list from original x
        ...
        if multi_label:
            ...
            indices = conf.view(-1) > conf_thres
            x = torch.cat((box, conf, j.float()), 1)[indices]
            remaining_indices = remaining_indices[indices]
