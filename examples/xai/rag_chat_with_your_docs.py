...
def chunk_pdf(document_name: str) -> list[tuple[str, int]]:
    ...
    for page_number, page in tqdm(
        enumerate(reader.pages, start=1), total=len(reader.pages), desc="Chunking Pages"
    ):
...
def load_data(collection: chromadb.Collection) -> None:
    ...
    for file in pdfs:
        ...
        collection.add(
            ...
            metadatas=[
                {"document_name": file, "page_number": chunk[1]} for chunk in chunks
            ],
        )
...
async def main():
    ...
    while True:
        ...
        context = collection.query(
            query_texts=[user_input], include=["documents"], n_results=5
        )["documents"][0]
        prompt_context = "\n\n".join(context)
        prompt = (
            f"User query: {user_input}. Answer using this context:\n\n {prompt_context}"
        )
