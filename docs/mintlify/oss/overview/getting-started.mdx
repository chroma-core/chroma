...
In this guide we used Chroma's [in-memory client](/oss/run-chroma/clients#in-memory-client) for simplicity. It starts a Chroma server in-memory, so any data you ingest will be lost when your program terminates. You can use the [persistent client](/oss/run-chroma/clients#persistent-client) or run Chroma in [client-server mode](/oss/run-chroma/client-server) if you need data persistence.
...
- We offer [first class support](/oss/embeddings/embedding-functions) for various embedding providers via our embedding function interface. Each embedding function ships in its own npm package.
- Learn how to [Deploy Chroma](/guides/deploy/client-server-mode) to a server
- Join Chroma's [Discord Community](https://discord.com/invite/MMeYNTmh3x) to ask questions and get help
- Follow Chroma on [X (@trychroma)](https://twitter.com/trychroma) for updates
