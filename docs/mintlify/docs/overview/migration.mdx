...
Auth changes in this section apply to the HTTP server path.
The Rust-backed embedded API uses a fixed server identity and does not run the
`ServerAuthenticationProvider` / `ServerAuthorizationProvider` middleware.
...
If you are using Chroma's built-in auth system, you will need to update your configuration and
