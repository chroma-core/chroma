...
<YouTube
  src="https://www.youtube.com/embed/RkI1SORvxdI?start=16"
...
Not every source deserves the same trust at write time. A memory extracted from a tool result, a web page or another agent's message can carry instructions that are replayed into every later run: memory poisoning, [ASI06](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/) in the OWASP Top 10 for Agentic Applications and [AML.T0080.000](https://atlas.mitre.org/techniques/AML.T0080.000) in MITRE ATLAS. Use the **Source** tag above to apply stricter validation to untrusted sources, screen values for embedded instructions and sensitive data before they are persisted, and keep integrity checks on memories that must not change (for example, procedural rules). [OWASP Agent Memory Guard](https://github.com/OWASP/www-project-agent-memory-guard) is an open-source guard that applies these checks on the write/read path and has adapters for LangChain, OpenAI Agents SDK, AutoGen, CrewAI and mem0.
