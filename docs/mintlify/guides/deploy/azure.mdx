...
### Step 4: Generate new Key Pairs
Generate a new key pair in the ssh folder:
- Windows: `C:\Users\<YourUsername>\.ssh\`
- Linux/macOS: `~/.ssh/` (i.e. `$HOME/.ssh/`)

Please choose a strong passphrase.
```bash
