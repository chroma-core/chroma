...
```bash
go get github.com/chroma-core/chroma/clients/go/pkg/embeddings/perplexity
```
...
// Create Perplexity embedding function
ef, err := perplexity.NewPerplexityEmbeddingFunction(
    perplexity.WithEnvAPIKey(), // reads PERPLEXITY_API_KEY
    perplexity.WithModel("pplx-embed-v1-4b"),
)
...
// Create a Chroma client and collection
client, err := chroma.NewHTTPClient(chroma.WithBaseURL("http://localhost:8000"))
...
results, err := collection.Query(ctx,
    chroma.WithQueryTexts("What programming languages are good for data science?"),
...
ef, err := perplexity.NewPerplexityEmbeddingFunction(
    ...
    perplexity.WithModel("pplx-embed-v1-4b"),
    perplexity.WithDimensions(512),
)

embeddings, err := ef.EmbedDocuments(ctx, []string{"document1", "document2"})
fmt.Println(len(embeddings[0].GetFloat32())) // 512
```
