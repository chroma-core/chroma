...
4. **Document operators**: `$regex` and `$not_regex` can only be used with the `#document` field. `$contains` and `$not_contains` can be used with both metadata array fields and `#document`, but have different semantics (array membership vs. substring matching).

5. **Numeric operators**: `$gt`, `$gte`, `$lt`, `$lte` can only be used with int or float values.

6. **Array types**: `$in` and `$nin` require arrays of the same type (all strings, all ints, all floats, or all booleans).

7. **Metadata array operators**: `$contains` and `$not_contains` on metadata fields require the field to be an array type (string[], int[], float[], or boolean[]), and check if the array contains the specified scalar value.
...
Check if an array metadata field contains a specific value:
...
```json
{
  "#document": {"$regex": "quantum\\s+computing"}
}
```
...
When both metadata and document filters are needed, combine them with `$and`:
