...
### max_marginal_relevance_query

Query using Maximal Marginal Relevance (MMR) to diversify results.

MMR over-fetches `fetch_k` nearest-neighbor candidates, then greedily selects
`n_results` records that balance relevance to the query against diversity among
the selected records. This helps retrieval workflows avoid returning several
near-duplicate chunks for the same query.
...
<ParamField path="n_results" type="int">
  Number of diversified neighbors to return per query.
</ParamField>

<ParamField path="fetch_k" type="int">
  Number of nearest-neighbor candidates to fetch before applying MMR. Chroma
  clamps this to at least `n_results`.
</ParamField>

<ParamField path="lambda_mult" type="float">
  Relevance/diversity trade-off in `[0, 1]`. `1.0` is pure relevance, while
  `0.0` applies the strongest diversity after the first pick.
</ParamField>
...
<ParamField path="include" type="List[Literal[documents, embeddings, metadatas, distances, uris, data]]">
  Fields to include in results. If embeddings are not requested, Chroma still
  fetches them internally for reranking and removes them from the returned
  payload.
</ParamField>

**Returns:** Diversified nearest-neighbor results.
