...
export default function MobileSidebar({currentSection, currentId}) {
    ...
    const curr = currentSection.subsections?.flatMap((subsection: any) => 
    ...
    return (
        ...
        <div 
            ...
            {currentSection.name}{curr?.name ? `: ${curr.name}` : ""}
            <ChevronDown className={`text-gray-500 transition-all ${menuOpen ? '': 'rotate-180'}`} />
        </div>
        
        <div className={`md:hidden flex flex-col pt-3 gap-2 w-full ${menuOpen ? "hidden": "block"} `}>
