...
pub struct PosthogClient {
    ...
    seen_event_types: HashSet<String>,
    buffer: Vec<Event>,
}
...
mod tests {
    ...
    async fn test_event_batching_logic() {
                ...
                match existing_event.batch(event2) {
                    Ok(()) => {
                        if existing_event.batch_size() >= max_batch_size2 {
                            ...
                            batched_events.insert(batch_key2, existing_event);
                        }
                    }
                    Err(e) => {
                        warn!(
        ...
        assert_eq!(batched_events[&batch_key1].batch_size(), 1);
        ...
        if max_batch_size3 != 1 && seen_event_types.contains(&batch_key3) {
            if let Some(mut existing_event) = batched_events.remove(&batch_key3) {
                match existing_event.batch(event3) {
                    Ok(()) => {
                        assert_eq!(existing_event.batch_size(), 2);
                        if existing_event.batch_size() >= max_batch_size3 {
                            mock_direct_capture(existing_event, &mut captured_events);
                        } else {
                            batched_events.insert(batch_key3, existing_event);
                        }
                    }
                    Err(e) => {
                        warn!(
        ...
        if max_batch_size4 != 1 && seen_event_types.contains(&batch_key4) {
            if let Some(mut existing_event) = batched_events.remove(&batch_key4) {
                if let Ok(()) = existing_event.batch(event4) {
                    assert_eq!(existing_event.batch_size(), 3);
                    if existing_event.batch_size() >= max_batch_size4 {
                        mock_direct_capture(existing_event, &mut captured_events);
                    }
                }
            }
