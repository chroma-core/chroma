...
pub async fn submit_event(event: Box<dyn ProductTelemetryEvent + Send + Sync>) {
    ...
    // If the global sender is available, use it
    if let Some(handler) = EVENT_SENDER.get() {
        if let Err(e) = handler.send(event, None).await {
...
mod tests {
    ...
    fn test_client_create_collection_event() {
        ...
        let event = ClientCreateCollectionEvent::new(uuid.clone(), embedding_function.clone());
