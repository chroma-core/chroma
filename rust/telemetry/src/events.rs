...
// Define the trait (only once)
pub trait ProductTelemetryEvent: Any + DowncastSync + Send + Sync {
    fn name(&self) -> String;
...
pub struct ClientStartEvent {
    pub in_colab: bool,
    // Note: We need a way to determine if we are in Colab in Rust.
    // For now, we'll hardcode false, but this needs a proper implementation.
}

impl ClientStartEvent {
    pub fn new() -> Self {
        // TODO: Implement actual Colab detection logic
        let is_in_colab = false; // Placeholder
        Self {
...
impl Default for ClientStartEvent {
    fn default() -> Self {
        Self::new()
    }
}

impl ProductTelemetryEvent for ClientStartEvent {
    fn name(&self) -> String {
        "ClientStartEvent".to_string()
    }
    fn properties(&self) -> HashMap<String, Value> {
    ...
    fn batch(
        ...
        Err("Batching not supported for ClientStartEvent")
    }
}
...
impl ServerStartEvent {
    pub fn new() -> Self {
        ...
        let is_cli =
            std::env::var("CHROMA_CLI").map_or(false, |val| val.eq_ignore_ascii_case("true"));
        Self { is_cli }
    }
}
...
impl ClientCreateCollectionEvent {
    pub fn new(collection_uuid: String) -> Self {
        Self { collection_uuid }
        // TODO: Add embedding function logic if required
    }
}
    ...
    fn batch(
        ...
        if let Some(other_event) = other.as_any().downcast_ref::<CollectionAddEvent>() {
            ...
            Ok(Box::new(CollectionAddEvent {
                collection_uuid: self.collection_uuid.clone(),
                add_amount: self.add_amount + other_event.add_amount,
                with_documents: self.with_documents + other_event.with_documents,
                with_metadata: self.with_metadata + other_event.with_metadata,
                with_uris: self.with_uris + other_event.with_uris,
                batch_size: self.batch_size + other_event.batch_size,
            }))
        } else {
...
impl ProductTelemetryEvent for CollectionUpdateEvent {
    ...
    fn batch(
        ...
        if let Some(other_event) = other.as_any().downcast_ref::<CollectionUpdateEvent>() {
            ...
            Ok(Box::new(CollectionUpdateEvent {
                collection_uuid: self.collection_uuid.clone(),
                update_amount: self.update_amount + other_event.update_amount,
                with_embeddings: self.with_embeddings + other_event.with_embeddings,
                with_metadata: self.with_metadata + other_event.with_metadata,
                with_documents: self.with_documents + other_event.with_documents,
                with_uris: self.with_uris + other_event.with_uris,
                batch_count: self.batch_count + other_event.batch_count,
            }))
        } else {
...
impl ProductTelemetryEvent for CollectionQueryEvent {
    ...
    fn batch(
        ...
        if let Some(other_event) = other.as_any().downcast_ref::<CollectionQueryEvent>() {
            ...
            Ok(Box::new(CollectionQueryEvent {
                collection_uuid: self.collection_uuid.clone(),
                query_amount: self.query_amount + other_event.query_amount,
                with_metadata_filter: self.with_metadata_filter + other_event.with_metadata_filter,
                with_document_filter: self.with_document_filter + other_event.with_document_filter,
                n_results: self.n_results + other_event.n_results,
                include_metadatas: self.include_metadatas + other_event.include_metadatas,
                include_documents: self.include_documents + other_event.include_documents,
                include_uris: self.include_uris + other_event.include_uris,
                include_distances: self.include_distances + other_event.include_distances,
                batch_count: self.batch_count + other_event.batch_count,
            }))
        } else {
            Err("Cannot batch events of different types")
...
impl ProductTelemetryEvent for CollectionGetEvent {
    ...
    fn batch_key(&self) -> String {
        format!("{}{}{:?}", self.collection_uuid, self.name(), self.limit)
    }
    ...
    fn batch(
        ...
        if let Some(other_event) = other.as_any().downcast_ref::<CollectionGetEvent>() {
            ...
            Ok(Box::new(CollectionGetEvent {
                collection_uuid: self.collection_uuid.clone(),
                ids_count: self.ids_count + other_event.ids_count,
                limit: self.limit, // Limit must be the same due to batch key
                include_metadata: self.include_metadata + other_event.include_metadata,
                include_documents: self.include_documents + other_event.include_documents,
                include_uris: self.include_uris + other_event.include_uris,
                batch_count: self.batch_count + other_event.batch_count,
            }))
...
mod tests {
    ...
    #[test]
    fn test_client_start_event() {
        let event = ClientStartEvent::new();
        assert_eq!(event.name(), "ClientStartEvent");
        assert_eq!(event.batch_key(), "ClientStartEvent");
        assert_eq!(event.max_batch_size(), 1);
        assert_eq!(event.batch_size(), 1);

        let properties = event.properties();
        assert_eq!(properties.get("in_colab").unwrap(), &json!(false));

        // Test batching not supported
        let other_event = Box::new(ClientStartEvent::new());
        assert!(event.batch(other_event).is_err());
    }

    #[test]
    fn test_server_start_event() {
        let event = ServerStartEvent::new();
    ...
    fn test_client_create_collection_event() {
        ...
        let event = ClientCreateCollectionEvent::new(uuid.clone());
        ...
        // Test batching not supported
        let other_event = Box::new(ClientCreateCollectionEvent::new("other-uuid".to_string()));
        assert!(event.batch(other_event).is_err());
    }
    ...
    fn test_collection_add_event() {
        let uuid = "test-uuid".to_string();
        let event = CollectionAddEvent::new(uuid.clone(), 10, 5, 3, 2, 1);
        ...
        let batched = event.batch(other_event).unwrap();
        let batched = batched
            .as_any()
            .downcast_ref::<CollectionAddEvent>()
            .unwrap();
        assert_eq!(batched.add_amount, 30);
        assert_eq!(batched.with_documents, 15);
        assert_eq!(batched.with_metadata, 9);
        assert_eq!(batched.with_uris, 6);
    ...
    fn test_collection_update_event() {
        let uuid = "test-uuid".to_string();
        let event = CollectionUpdateEvent::new(uuid.clone(), 10, 5, 3, 2, 1);
        ...
        let batched = batched
            .as_any()
            .downcast_ref::<CollectionUpdateEvent>()
            .unwrap();
        assert_eq!(batched.update_amount, 30);
        assert_eq!(batched.with_embeddings, 15);
        assert_eq!(batched.with_metadata, 9);
        assert_eq!(batched.with_documents, 6);
        assert_eq!(batched.with_uris, 3);
    ...
    fn test_collection_query_event() {
        let uuid = "test-uuid".to_string();
        let event = CollectionQueryEvent::new(uuid.clone(), 10, 5, 3, 2, 1, 1, 1, 1);
        ...
        let batched = batched
            .as_any()
            .downcast_ref::<CollectionQueryEvent>()
            .unwrap();
        assert_eq!(batched.query_amount, 30);
        assert_eq!(batched.with_metadata_filter, 15);
        assert_eq!(batched.with_document_filter, 9);
        assert_eq!(batched.n_results, 6);
        assert_eq!(batched.include_metadatas, 3);
        assert_eq!(batched.include_documents, 3);
        assert_eq!(batched.include_uris, 3);
        assert_eq!(batched.include_distances, 3);
        assert_eq!(batched.batch_count, 2);
    ...
    fn test_collection_get_event() {
        let uuid = "test-uuid".to_string();
        let event = CollectionGetEvent::new(uuid.clone(), 10, Some(5), 3, 2, 1);
        assert_eq!(event.name(), "CollectionGetEvent");
        assert_eq!(
            event.batch_key(),
            format!("{}{}{:?}", uuid, "CollectionGetEvent", Some(5))
        );
        ...
        assert_eq!(properties.get("limit").unwrap(), &json!(5));
        assert_eq!(properties.get("include_metadata").unwrap(), &json!(3));
        assert_eq!(properties.get("include_documents").unwrap(), &json!(2));
        assert_eq!(properties.get("include_uris").unwrap(), &json!(1));

        // Test successful batching
        let other_event = Box::new(CollectionGetEvent::new(uuid.clone(), 20, Some(5), 6, 4, 2));
        let batched = event.batch(other_event).unwrap();
        let batched = batched
            .as_any()
            .downcast_ref::<CollectionGetEvent>()
            .unwrap();
        assert_eq!(batched.ids_count, 30);
        assert_eq!(batched.limit, Some(5));
        assert_eq!(batched.include_metadata, 9);
        assert_eq!(batched.include_documents, 6);
        assert_eq!(batched.include_uris, 3);
        assert_eq!(batched.batch_count, 2);
    ...
    fn test_collection_delete_event() {
        let uuid = "test-uuid".to_string();
        let event = CollectionDeleteEvent::new(uuid.clone(), 10);
