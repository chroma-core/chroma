...
use arrow::{
    array::{
        Array, ArrayRef, FixedSizeListArray, FixedSizeListBuilder, ListArray, ListBuilder,
        PrimitiveArray, StructArray, UInt32Builder, UInt8Array, UInt8Builder,
    },
    datatypes::{ArrowPrimitiveType, DataType, Field, Fields, UInt32Type, UInt8Type},
};
use chroma_types::hierarchical_spann::{
    HierarchicalSpannPostingList, HierarchicalSpannPostingListOwned,
};

use crate::{
    arrow::{
        block::delta::{
            hierarchical_posting_list_delta::HierarchicalPostingListDelta, BlockStorage,
            UnorderedBlockDelta,
        },
...
const CODES_COLUMN: usize = 0;
const IDS_COLUMN: usize = 1;
const VERSIONS_COLUMN: usize = 2;
...
#[derive(Clone)]
pub struct HierarchicalPostingListSizeTracker {
    pub cluster_count: usize,
    pub code_length: usize,
    pub vector_count: usize,
}

pub struct HierarchicalPostingListArrowBuilder {
    codes: ListBuilder<FixedSizeListBuilder<UInt8Builder>>,
    ids: ListBuilder<UInt32Builder>,
    versions: ListBuilder<UInt8Builder>,
}

impl ArrowWriteableValue for HierarchicalSpannPostingList<'_> {
    type ReadableValue<'data> = HierarchicalSpannPostingList<'data>;
    type PreparedValue = HierarchicalSpannPostingListOwned;
    type SizeTracker = HierarchicalPostingListSizeTracker;
    type ArrowBuilder = HierarchicalPostingListArrowBuilder;
    ...
    fn add(prefix: &str, key: KeyWrapper, value: Self, delta: &BlockStorage) {
        match delta {
            BlockStorage::HierarchicalPostingListDelta(d) => d.add(prefix, key, value),
            _ => unreachable!("Invalid delta type for HierarchicalSpannPostingList"),
        }
    }

    fn delete(prefix: &str, key: KeyWrapper, delta: &UnorderedBlockDelta) {
        match &delta.builder {
            BlockStorage::HierarchicalPostingListDelta(d) => d.delete(prefix, key),
            _ => unreachable!("Invalid delta type for HierarchicalSpannPostingList"),
        }
    }

    fn get_delta_builder(_: BlockfileWriterMutationOrdering) -> BlockStorage {
        BlockStorage::HierarchicalPostingListDelta(HierarchicalPostingListDelta::new())
    }

    fn get_arrow_builder(tracker: Self::SizeTracker) -> Self::ArrowBuilder {
        HierarchicalPostingListArrowBuilder {
            ...
            versions: ListBuilder::with_capacity(
                UInt8Builder::with_capacity(tracker.vector_count),
                tracker.cluster_count,
            ),
        }
    }
    ...
    fn finish(
        ...
        let struct_fields = Fields::from(vec![codes_field, ids_field, versions_field]);
    ...
    fn get_owned_value_from_delta(
        ...
        match &delta.builder {
            BlockStorage::HierarchicalPostingListDelta(d) => d.get_owned_value(prefix, key),
            _ => unreachable!("Invalid delta type for HierarchicalSpannPostingList"),
        }
    }
}

impl<'data> ArrowReadableValue<'data> for HierarchicalSpannPostingList<'data> {
    fn get(array: &'data Arc<dyn Array>, index: usize) -> Self {
        ...
        HierarchicalSpannPostingList {
            codes,
            ids: get_list_slice::<UInt32Type>(ids_arr, index),
            versions: get_list_slice::<UInt8Type>(versions_arr, index),
        }
    }
    ...
    fn add_to_delta<K: ArrowWriteableKey>(
        ...
        <HierarchicalSpannPostingList as ArrowWriteableValue>::add(
            prefix,
            key.into(),
            value,
            storage,
