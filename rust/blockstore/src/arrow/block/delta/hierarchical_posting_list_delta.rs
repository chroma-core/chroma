...
use crate::{
    arrow::{
        block::value::hierarchical_posting_list_value::HierarchicalPostingListSizeTracker,
...
#[derive(Debug)]
struct Inner {
    storage: BTreeMap<CompositeKey, HierarchicalSpannPostingListOwned>,
    key_size: usize,
    prefix_size: usize,
    vector_count: usize,
}
...
#[derive(Debug, Clone)]
pub struct HierarchicalPostingListDelta {
    inner: Arc<RwLock<Inner>>,
}

impl HierarchicalPostingListDelta {
    ...
    pub fn get_owned_value(
        ...
        prefix: &str,
        key: KeyWrapper,
    ) -> Option<HierarchicalSpannPostingListOwned> {
    ...
    pub fn add(&self, prefix: &str, key: KeyWrapper, value: HierarchicalSpannPostingList<'_>) {
    ...
    pub(super) fn get_size<K: ArrowWriteableKey>(&self) -> usize {
        ...
        let code_length = inner
            .storage
            .values()
            .next()
            .map(|v| v.codes.len() / v.ids.len().max(1))
            .unwrap_or(0);

        // Arrow size breakdown (each component 64-byte aligned):
        // - prefix_size / key_size: composite key strings and data
        // - codes:    List<FixedSizeList<u8>>, vector_count * code_length bytes
        // - ids:      List<u32>, vector_count * 4 bytes
        // - versions: List<u8>,  vector_count bytes
        // - offsets:  3 outer-list offset arrays (codes, ids, versions), each (cluster_count+1) i32
    ...
    pub(super) fn split<K: ArrowWriteableKey>(
        &self,
        split_size: usize,
    ) -> (CompositeKey, HierarchicalPostingListDelta) {
    ...
    fn find_split_key<K: ArrowWriteableKey>(&self, split_size: usize) -> CompositeKey {
        ...
        while let Some((k, v)) = iter.next() {
            ...
            let total_size = bit_util::round_upto_multiple_of_64(prefix_size)
                ...
                + bit_util::round_upto_multiple_of_64(vector_count * code_length * size_of::<u8>())
                + bit_util::round_upto_multiple_of_64(vector_count * size_of::<u32>())
                + bit_util::round_upto_multiple_of_64(vector_count * size_of::<u8>())
                + bit_util::round_upto_multiple_of_64((cluster_count + 1) * 4) * 3
                + K::offset_size(cluster_count);
    ...
    pub(super) fn into_arrow(
        ...
        let inner = Arc::try_unwrap(self.inner)
            .expect("HierarchicalPostingListDelta inner should have only one reference")
            .into_inner();
        ...
        let size_tracker = HierarchicalPostingListSizeTracker {
        ...
        let mut value_builder =
            <HierarchicalSpannPostingList as ArrowWriteableValue>::get_arrow_builder(
                size_tracker.clone(),
            );

        for (key, value) in inner.storage.into_iter() {
            key_builder.add_key(key);
            <HierarchicalSpannPostingList as ArrowWriteableValue>::append(
                value,
                &mut value_builder,
            );
        }
        ...
        let (struct_field, value_arr) =
            <HierarchicalSpannPostingList as ArrowWriteableValue>::finish(
                value_builder,
                &size_tracker,
            );
