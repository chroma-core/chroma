...
impl Default for BlockFileMetrics {
    fn default() -> Self {
        let meter = global::meter("chroma");
        Self {
            commit_latency: meter
                .u64_histogram("blockfile_commit_latency")
                .with_description("Commit latency")
                .with_unit("microseconds")
                .build(),
            num_blocks_flushed: meter
                .u64_histogram("blockfile_num_blocks_flushed")
                .with_description("Number of blocks flushed")
                .with_unit("blocks")
                .build(),
            flush_latency: meter
                .u64_histogram("blockfile_flush_latency")
...
pub(super) struct BlockManager {
    ...
    storage: Storage,
    max_block_size_bytes: usize,
    blockfile_metrics: BlockFileMetrics,
}
...
impl BlockManager {
    pub(super) fn new(
        ...
        Self {
            ...
            blockfile_metrics: BlockFileMetrics::default(),
        }
    }
    ...
    pub(super) async fn commit<K: ArrowWriteableKey, V: ArrowWriteableValue>(
        ...
        let _stopwatch = Stopwatch::new(&self.blockfile_metrics.commit_latency, &attribute);
    ...
    pub(super) async fn flush(&self, block: &Block) -> Result<(), Box<dyn ChromaError>> {
        ...
        let trace_id = get_current_trace_id().to_string();
        let attribute = [KeyValue::new("trace_id", trace_id)];
        let _stopwatch = Stopwatch::new(&self.blockfile_metrics.flush_latency, &attribute);
