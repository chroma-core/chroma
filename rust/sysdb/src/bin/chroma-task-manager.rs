...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Command::GetAttachedFunction {
            ...
            let mut attached_functions = response.into_inner().attached_functions;
            if attached_functions.is_empty() {
                return Err("Attached function not found".into());
            }
            let attached_function = attached_functions.remove(0);
