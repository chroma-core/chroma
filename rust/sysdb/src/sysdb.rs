...
impl GrpcSysDb {
    ...
    pub async fn create_attached_function(
        ...
        let attached_function = Self::attached_function_from_proto(attached_function_proto)
            .map_err(|e| {
                AttachFunctionError::FailedToCreateAttachedFunction(tonic::Status::internal(
                    format!("Failed to convert attached function: {:?}", e),
                ))
            })?;
...
impl SysDb {
    ...
    pub async fn get_attached_function_with_info_by_name(
        ...
        let attached_function = match self {
            SysDb::Grpc(grpc) => {
                grpc.get_attached_function_by_name(
                    input_collection_id,
        ...
        let function = match self {
            SysDb::Grpc(grpc) => {
                ...
                chroma_types::Function {
                    id: uuid::Uuid::parse_str(&function_proto.id)
                        .map_err(|_| GetAttachedFunctionError::ServerReturnedInvalidData)?,
