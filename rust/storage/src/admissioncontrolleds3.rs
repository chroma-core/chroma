...
pub struct RollbackPriorityOnDrop(InflightRequestContext, StorageRequestPriority);
...
impl CountBasedPolicy {
    ...
    async fn acquire(
        ...
        let priority_and_hostname_attr = [
            KeyValue::new(
                "priority",
                priority.get_priority().unwrap().as_usize().to_string(),
            ),
            self.metrics.hostname_attribute[0].clone(),
        ];
        self.metrics.nac_available_permits.record(
            self.remaining_tokens[priority.get_priority().unwrap().as_usize()].available_permits()
                as u64,
