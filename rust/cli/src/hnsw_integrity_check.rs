...
use chroma_segment::local_hnsw::{
    inspect_persisted_hnsw_index, inspect_persisted_hnsw_metadata, parse_persisted_hnsw_dim,
    PersistedHnswLabelMismatch, PersistedHnswMetadata, HNSW_HEADER_FILE, HNSW_INDEX_FILES,
    METADATA_FILE,
};
...
fn inspect_segment(
    ...
    if HNSW_INDEX_FILES
        .iter()
        .all(|name| index_dir.join(name).is_file())
    {
        match inspect_persisted_hnsw_index(&index_dir) {
            Ok(index)
                if !has_durable_watermark
                    && index.elements == 0
                    && !index_dir.join(METADATA_FILE).exists() =>
            {
                // Native initialization creates files before the first checkpoint.
            ...
            Err(err) => push_issue(
                ...
                segment,
                log_state.clone(),
                format!("invalid persisted HNSW index: {err}"),
            ),
        }
    }
