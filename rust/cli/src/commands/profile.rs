...
use crate::utils::{get_or_create_credentials_file, parse_credentials_file, read_cli_config, write_cli_config, write_credentials_file};
...
fn list_profiles() {
    println!("{}", "\nAvailable profiles:\n".blue().bold());
    let credentials_file = get_or_create_credentials_file();
    let profiles = parse_credentials_file(&credentials_file).expect("\nCould not parse credentials file");
