...
use crate::utils::{get_or_create_credentials_file, parse_credentials_file, write_cli_config, write_credentials_file, Profile};
...
pub fn login(args: LoginArgs) {
    ...
    let mut profiles = parse_credentials_file(&credentials_file).expect("\nCould not parse credentials file\n");
    ...
    profiles.insert(profile.clone(), Profile { api_key: api_key.clone(), tenant_id: tenant_id.clone() });
    
    if !has_default && profile != "default" {
        println!("{} {} {}", "\nProfile".blue().bold(), profile.clone().bold().blue(), "saved as your default".blue().bold());
        profiles.insert("default".to_string(), Profile { api_key: api_key.clone(), tenant_id: tenant_id.clone() });
    }
