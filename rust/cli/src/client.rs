...
pub fn chroma_server_get_request<T: DeserializeOwned>(
    ...
    let result = chroma_server_request::<T, ()>(RequestMethod::Get, api_url, route, api_key, None)?;
...
pub fn chroma_server_delete_request(
    ...
    chroma_server_request::<(), ()>(RequestMethod::Delete, api_url, route, api_key, None)?;
...
pub fn chroma_server_post_request<T: DeserializeOwned, U: Serialize>(
    ...
    let result =
        chroma_server_request::<T, U>(RequestMethod::Post, api_url, route, api_key, Some(body))?;
...
pub fn create_database(
    ...
    let create_db_route = format!("/api/v2/tenants/{}/databases", profile.team_id);

    chroma_server_post_request::<EmptyResponse, CreateDatabasePayload>(
        api_url,
        &create_db_route,
        &profile.api_key,
...
pub fn list_databases(api_url: &str, profile: &Profile) -> Result<(Vec<Database>), Box<dyn Error>> {
    ...
    let response = chroma_server_get_request::<ListDatabasesResponse>(
        api_url,
        &list_dbs_route,
        &profile.api_key,
    )?;
