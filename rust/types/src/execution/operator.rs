...
impl RankExpr {
    ...
    pub fn normalize_sparse_vectors(&mut self) {
        match self {
            RankExpr::Knn { query, .. } => {
                if let QueryVector::Sparse(sv) = query {
                    *sv = sv.normalize();
                }
            }
            RankExpr::Absolute(expr)
            | RankExpr::Exponentiation(expr)
            | RankExpr::Logarithm(expr) => expr.normalize_sparse_vectors(),
            RankExpr::Division { left, right } | RankExpr::Subtraction { left, right } => {
                left.normalize_sparse_vectors();
                right.normalize_sparse_vectors();
            }
            RankExpr::Maximum(exprs)
            | RankExpr::Minimum(exprs)
            | RankExpr::Multiplication(exprs)
            | RankExpr::Summation(exprs) => {
                exprs
                    .iter_mut()
                    .for_each(RankExpr::normalize_sparse_vectors);
            }
            RankExpr::Value(_) => {}
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_rank_expr_normalize_sparse_vectors() {
        use crate::metadata::SparseVector;

        // Create unsorted sparse vector
        let sparse = SparseVector::new(vec![3, 1, 2], vec![0.3, 0.1, 0.2]);
        let mut expr = RankExpr::Knn {
        ...
        // Check that sparse vector is now sorted
        if let RankExpr::Knn {
            query: QueryVector::Sparse(sv),
            ..
        } = expr
        {
            ...
            panic!("Expected Knn with Sparse vector");
        }
    }

    #[test]
    fn test_rank_expr_normalize_nested() {
        use crate::metadata::SparseVector;

        // Create nested expression with unsorted sparse vector
        let sparse = SparseVector::new(vec![5, 2, 4], vec![0.5, 0.2, 0.4]);
        let knn = Box::new(RankExpr::Knn {
            query: QueryVector::Sparse(sparse),
            key: "#embedding".to_string(),
            limit: 10,
            default: None,
        ...
        let mut expr = RankExpr::Logarithm(knn);
        expr.normalize_sparse_vectors();

        // Check that nested sparse vector is normalized
        if let RankExpr::Logarithm(inner) = expr {
            if let RankExpr::Knn {
                query: QueryVector::Sparse(sv),
                ..
            } = inner.as_ref()
            {
                assert_eq!(sv.indices, vec![2, 4, 5]);
                assert_eq!(sv.values, vec![0.2, 0.4, 0.5]);
            } else {
                panic!("Expected nested Knn with Sparse vector");
            }
        } else {
            panic!("Expected Logarithm expression");
        }
    }

    #[test]
    fn test_rank_expr_normalize_multiple() {
        use crate::metadata::SparseVector;

        // Create expression with multiple sparse vectors
        let sparse1 = SparseVector::new(vec![3, 1], vec![0.3, 0.1]);
        let sparse2 = SparseVector::new(vec![4, 2], vec![0.4, 0.2]);

        let knn1 = RankExpr::Knn {
            query: QueryVector::Sparse(sparse1),
            key: "#embedding".to_string(),
            limit: 10,
            default: None,
            return_rank: false,
        ...
        let knn2 = RankExpr::Knn {
            query: QueryVector::Sparse(sparse2),
            key: "#embedding".to_string(),
            limit: 10,
            default: None,
            return_rank: false,
        ...
        let mut expr = RankExpr::Summation(vec![knn1, knn2]);
        expr.normalize_sparse_vectors();

        // Check that both sparse vectors are normalized
        if let RankExpr::Summation(exprs) = expr {
            if let RankExpr::Knn {
                query: QueryVector::Sparse(sv),
                ..
            } = &exprs[0]
            {
                ...
                assert_eq!(sv.values, vec![0.1, 0.3]);
            }
            if let RankExpr::Knn {
                query: QueryVector::Sparse(sv),
                ..
            ...
            {
                assert_eq!(sv.indices, vec![2, 4]);
                assert_eq!(sv.values, vec![0.2, 0.4]);
            }
        } else {
            panic!("Expected Summation expression");
        }
    }
}
