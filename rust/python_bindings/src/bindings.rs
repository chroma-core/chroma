...
impl Bindings {
    ...
    #[pyo3(
        signature = (collection_id, ids = None, limit = 10, seed = None, include = ["metadatas".to_string(), "documents".to_string()].to_vec(), tenant = DEFAULT_TENANT.to_string(), database = DEFAULT_DATABASE.to_string())
    )]
    #[allow(clippy::too_many_arguments)]
    fn sample(
        &self,
        ...
        let request =
            SampleRequest::try_new(tenant, database, collection_id, ids, limit, seed, include)?;
