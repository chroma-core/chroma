...
use chroma_types::{
    ...
    Metadata, QueryResponse, SegmentType, UpdateCollectionConfiguration, UpdateCollectionRequest,
    UpdateMetadata, VectorIndexConfiguration, WrappedSerdeJsonError,
};
...
impl Bindings {
    ...
    fn create_collection(
        ...
        if let Some(config) = configuration.as_ref() {
            ...
            {
                return Err(WrappedPyErr::from(PyValueError::new_err(
                    "SPANN is still in development. Not allowed to created spann indexes",
                ))
                .into());
            }
        }
