...
use crate::{
    ...
    contexts::{generate_context_impls, generate_dispatch_impl},
...
pub fn initialize_metering(raw_token_stream: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let (capabilities, contexts, passthrough_items) = match process_token_stream(&token_stream) {
        Ok(result) => result,
    ...
    let marker_method_definitions: Vec<TokenStream> = capabilities
        .iter()
        .map(generate_capability_marker_method_definition)
        .collect();

    let context_impl_blocks: Vec<TokenStream> = contexts
        .iter()
        .map(|context| generate_context_impls(context, &capability_id_to_capability))
        .collect();

    let dispatch_impls: Vec<TokenStream> =
        capabilities.iter().map(generate_dispatch_impl).collect();
