...
pub enum Base64DecodeError {
    ...
    #[error("Failed to convert chunk {chunk_index} to byte array")]
    ChunkConversionFailed { chunk_index: usize },
}

#[derive(Serialize, Deserialize, ToSchema, Debug, Clone)]
#[serde(untagged)]
pub enum EmbeddingsFormat {
...
#[derive(Serialize, Deserialize, ToSchema, Debug, Clone)]
#[serde(untagged)]
pub enum UpdateEmbeddingsFormat {
    JsonArrays(Vec<Option<Vec<f32>>>),
    Base64Binary(Vec<Option<String>>),
}

pub(crate) fn get_embeddings(
    embeddings: Option<EmbeddingsFormat>,
) -> Result<Option<Vec<Vec<f32>>>, ValidationError> {
    match embeddings {
        Some(EmbeddingsFormat::Base64Binary(base64_strings)) => {
            Ok(Some(decode_base64_embeddings(&base64_strings)?))
        }
        Some(EmbeddingsFormat::JsonArrays(arrays)) => Ok(Some(arrays)),
        None => Ok(None),
    }
}

pub(crate) fn get_update_embeddings(
    embeddings: Option<UpdateEmbeddingsFormat>,
) -> Result<Option<Vec<Option<Vec<f32>>>>, ValidationError> {
    match embeddings {
        Some(UpdateEmbeddingsFormat::Base64Binary(base64_data)) => {
            Ok(Some(decode_base64_update_embeddings(&base64_data)?))
        }
        Some(UpdateEmbeddingsFormat::JsonArrays(arrays)) => Ok(Some(arrays)),
        None => Ok(None),
    }
}
...
pub fn decode_base64_embedding(base64_str: &String) -> Result<Vec<f32>, Base64DecodeError> {
    ...
    for (chunk_index, chunk) in bytes.chunks_exact(4).enumerate() {
        let float_bytes: [u8; 4] = chunk
            .try_into()
            .map_err(|_| Base64DecodeError::ChunkConversionFailed { chunk_index })?;
...
mod tests {
    ...
    fn test_get_embeddings_propagates_error() {
        let invalid_embeddings = EmbeddingsFormat::Base64Binary(vec!["invalid!@#$".to_string()]);
        let result = get_embeddings(Some(invalid_embeddings));
    ...
    fn test_multiple_embeddings_with_one_invalid() {
        ...
        let embeddings =
            EmbeddingsFormat::Base64Binary(vec![valid_base64, "invalid!@#$".to_string()]);

        let result = get_embeddings(Some(embeddings));
        assert!(matches!(
            result,
            Err(ValidationError::Base64Decode(
                Base64DecodeError::InvalidBase64(_)
