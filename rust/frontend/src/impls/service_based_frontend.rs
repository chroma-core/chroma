...
mod tests {
    ...
    async fn test_default_sqlite_segments() {
        ...
        assert!(segments
            .iter()
            .find(|s| s.r#type == SegmentType::Sqlite && s.scope == SegmentScope::METADATA)
            .is_some());
        assert!(
            segments
                .iter()
                .find(|s| s.r#type == SegmentType::HnswLocalPersisted
    ...
    async fn test_k8s_integration_default_distributed_segments() {
        ...
        assert!(segments
            .iter()
            .find(|s| s.r#type == SegmentType::HnswDistributed && s.scope == SegmentScope::VECTOR)
            .is_some());
        assert!(segments
            .iter()
            .find(|s| s.r#type == SegmentType::BlockfileRecord && s.scope == SegmentScope::RECORD)
            .is_some());
    }
}
