...
use chroma_types::{
    ...
    AttachFunctionRequest, Collection, CollectionUuid, CountCollectionsError,
    ...
    ForkCollectionResponse, GetAttachedFunctionError, GetAttachedFunctionResponse,
    ...
    QueryRequest, QueryResponse, ResetError, ResetResponse, Schema, SchemaError, SearchRequest,
    ...
    pub async fn attach_function(
        ...
        let input_collection = self
            ...
            .ok_or(chroma_types::AttachFunctionError::Internal(Box::new(
                chroma_error::TonicError(tonic::Status::internal(
                    "Input collection not found after attaching function",
                )),
            )))?;

        // Format timestamps as strings (same as get_attached_function)
        let last_run = attached_function_with_info
            .attached_function
            .last_run
            .map(|time| {
        ...
        let next_run = attached_function_with_info
            .attached_function
            .next_run
        ...
        Ok(GetAttachedFunctionResponse {
            attached_function: AttachedFunctionInfo {
                ...
                output_collection: attached_function_with_info
                    .attached_function
                    .output_collection_name,
                last_run,
                next_run,
                params: attached_function_with_info.attached_function.params,
                global_function_parent: attached_function_with_info
                    .attached_function
                    .global_parent
                    .map(|u| u.to_string()),
            },
        ...
        let attached_function_with_info = self
            ...
            .map_err(|e| match e {
                ...
                chroma_sysdb::GetAttachedFunctionError::FailedToGetAttachedFunction(s) => {
                    GetAttachedFunctionError::FailedToGetAttachedFunction(Box::new(
                        chroma_error::TonicError(s),
                    )
