...
impl ManifestManager {
    pub fn new(spanner: Arc<Client>, regions: Vec<String>, log_id: Uuid) -> Self {
        Self {
            spanner,
            regions,
            log_id,
...
mod tests {
    ...
    async fn test_k8s_mcmr_integration_manifest_head_matches() {
        ...
        let manager = ManifestManager::new(Arc::new(client), vec!["dummy".to_string()], log_id);
    ...
    async fn test_k8s_mcmr_integration_load_with_fragments() {
        ...
        let manager =
            ManifestManager::new(Arc::new(client.clone()), vec!["dummy".to_string()], log_id);
    ...
    async fn test_k8s_mcmr_integration_recover() {
        ...
        let mut manager = ManifestManager::new(Arc::new(client), vec!["dummy".to_string()], log_id);
    ...
    async fn test_k8s_mcmr_integration_publish_fragment_concurrent() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                let manager = ManifestManager::new(client, vec!["dummy".to_string()], log_id);
                let pointer = FragmentUuid::generate();
