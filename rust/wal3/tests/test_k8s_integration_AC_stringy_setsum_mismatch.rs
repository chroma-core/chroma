...
async fn test_k8s_integration_ab_stringy_setsum_mismatch() {
    ...
    let position5 = log.append(vec![50, 51, 52, 53]).await.unwrap();
    let fragment5 = FragmentCondition {
        path: "log/Bucket=0000000000000000/FragmentSeqNo=0000000000000005.parquet".to_string(),
        seq_no: FragmentSeqNo(5),
        start: 5,
        limit: 6,
        num_bytes: 1044,
        data: vec![(position5, vec![50, 51, 52, 53])],
    };
    let position6 = log.append(vec![60, 61, 62, 63]).await.unwrap();
    let fragment6 = FragmentCondition {
        path: "log/Bucket=0000000000000000/FragmentSeqNo=0000000000000006.parquet".to_string(),
        seq_no: FragmentSeqNo(6),
        start: 6,
        limit: 7,
        num_bytes: 1044,
        data: vec![(position6, vec![60, 61, 62, 63])],
    };
    ...
    let postconditions = [
        Condition::Manifest(ManifestCondition {
            ...
            snapshots: vec![SnapshotCondition {
                ...
                limit: LogPosition::from_offset(5),
                num_bytes: 5166,
                writer: "test writer".to_string(),
                snapshots: vec![
                    SnapshotCondition {
            ...
            fragments: vec![fragment5.clone(), fragment6.clone()],
        }),
        ...
        Condition::Fragment(fragment5.clone()),
        Condition::Fragment(fragment6.clone()),
    ];
    assert_conditions(
        &storage,
        "test_k8s_integration_AB_stringy_setsum_mismatch",
    ...
    log.garbage_collect_phase3_delete_garbage(&GarbageCollectionOptions::default())
        .await
        .unwrap();
    panic!("EXPECTED TO FAIL---MAKE IT FAIL, FIX IT, REMOVE THIS");
}
