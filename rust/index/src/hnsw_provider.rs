...
impl HnswIndexProvider {
    ...
    pub async fn fork(
        ...
        match self.get(&new_id, cache_key).await {
            ...
            None => {
                ...
                // Capture the values we need before creating the closure
                let use_direct_hnsw = self.use_direct_hnsw;
                let cache = self.cache.clone();
                let cache_key_owned = *cache_key;
                let prefix_path_owned = prefix_path.to_string();
                let temporary_storage_path = self.temporary_storage_path.clone();
                let source_id_owned = *source_id;
                // fetch callbacks need to return a StorageError

                let hnsw_data_processor = async move |hnsw_data: hnswlib::HnswData| {
                    let index_raw = if use_direct_hnsw {
                        HnswIndex::load_from_hnsw_data(hnsw_data, &index_config, ef_search, new_id)
                    ...
                    let index = HnswIndexRef {
                        inner: Arc::new(RwLock::new(DistributedHnswInner {
                            ...
                            prefix_path: prefix_path_owned,
                        })),
                    };
                    cache.insert(cache_key_owned, index.clone()).await;
                    Ok(index)
                };

                let index = self
                    .fetch_hnsw_segment(source_id, prefix_path, hnsw_data_processor)
                    .await
                    .map_err(|e| {
    ...
    async fn fetch_hnsw_segment<FinishFn, FetchFut, FinishRet>(
        &self,
        source_id: &IndexUuid,
        prefix_path: &str,
        hnsw_data_processor: FinishFn,
    ) -> Result<FinishRet, chroma_storage::StorageError>
    where
        FinishFn: FnOnce(hnswlib::HnswData) -> FetchFut + Send + 'static,
        FetchFut: Future<Output = Result<FinishRet, chroma_storage::StorageError>> + Send + 'static,
        FinishRet: Clone + Sync + Send + 'static,
    {
        // array of length(files)
        let atomic_buffer_count = Arc::new(AtomicUsize::new(0));
        let buffers: Arc<[OnceLock<Arc<Vec<u8>>>]> = Arc::from(
            (0..FILES.len())
                .map(|_| OnceLock::new())
                .collect::<Vec<_>>(),
        );

        // Since only one thread will call the processor, we can use a simple Mutex
        let processor = Arc::new(std::sync::Mutex::new(Some(hnsw_data_processor)));

        // Fetch all files in parallel
        let futures = FILES.iter().enumerate().map(|(index, file)| {
            let prefix_path = prefix_path.to_string();
            let atomic_buffer_count = atomic_buffer_count.clone();
                        ...
                        let fetch_res = self
                            ...
                            .fetch(
                                ...
                                move |buf| {
                                    ...
                                    let processor = processor.clone();
                                    async move {
                                        let buf = buf.expect("Failed to fetch hnsw index file");
                                        buffers[index].set(buf).map_err(|_| {
                                            chroma_storage::StorageError::Generic {
                                        ...
                                        atomic_buffer_count.fetch_add(1, Ordering::SeqCst);
                                        if atomic_buffer_count.load(Ordering::SeqCst) == FILES.len() {
                                            // Only one thread will process the final result
                                            let res = match hnswlib::HnswDataBuilder::new()
                                                .header_buffer(buffers[0].get().unwrap().clone())
                                                .data_level0_buffer(buffers[1].get().unwrap().clone())
                                                .length_buffer(buffers[2].get().unwrap().clone())
                                                .link_list_buffer(buffers[3].get().unwrap().clone())
                                                .build()
                                            {
                                                Ok(hnsw_data) => {
                                                    // Take the processor and call it (only one thread will reach here)
                                                    let processor_fn = processor.lock().unwrap().take().unwrap();
                                                    Ok(Some(processor_fn(hnsw_data).await?))
                                                },
                                                Err(e) => Err(chroma_storage::StorageError::Generic {
        ...
        // find the first Ok(Some(_))
        let return_val = results.into_iter().find_map(|(fetch_result, _etag)| {
        ...
        Ok(return_val.expect("At least one thread should have returned a result"))
    }
            ...
            None => {
                ...
                let hnsw_data_processor = async move |hnsw_data: hnswlib::HnswData| {
                    let index_raw = if use_direct_hnsw {
                        ...
                        Self::load_hnsw_data_through_disk_and_purge(
                            &temporary_storage_path,
                            &id_owned,
                            id_owned,
                            &prefix_path_owned,
                ...
                let index = self
                    .fetch_hnsw_segment(id, prefix_path, hnsw_data_processor)
                    .map_err(|e| {
