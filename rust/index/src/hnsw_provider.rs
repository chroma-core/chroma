            ...
            .fetch_batch(
                ...
                |bufs| async move {
                    ...
                    let hnsw_data = hnswlib::HnswData::builder()
                        .header_buffer(bufs[0].as_ref().unwrap().clone())
                        .data_level0_buffer(bufs[1].as_ref().unwrap().clone())
                        .length_buffer(bufs[2].as_ref().unwrap().clone())
                        .link_list_buffer(bufs[3].as_ref().unwrap().clone())
                        .build();
                    if let Ok(hnsw_data) = hnsw_data {
                        hnsw_data_processor(hnsw_data).await
                    } else {
                        Err(chroma_storage::StorageError::CallbackError {
                            info: hnsw_data.err().unwrap().to_string(),
                        })
                    }
                },
            )
