...
impl FullTextIndexWriter {
    ...
    pub async fn prefetch_before_writing_to_blockfiles(
        &self,
        ...
        if let Some(id) = self.posting_lists_blockfile_writer.forked_from_id() {
            let mut keys = vec![];
            {
                ...
                for group in token_instances.iter() {
                    ...
                    for i in 0..num_samples {
                        let idx = (i as f64 * (group.len() - 1) as f64
                            / (num_samples - 1).max(1) as f64)
                            .round() as usize;
            ...
            tracing::info!("Prefetching {} keys", keys.len());
            let reader = blockfile_provider.read::<u32, &[u32]>(&id).await.unwrap();
            reader.load_blocks_for_keys(keys).await;
        } else {
            tracing::info!("Skipping prefetch since this blockfile was not forked");
        }
