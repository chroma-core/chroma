...
fn print_error_analysis() {
    ...
    let codes_1: Vec<Vec<u8>> = embeddings
        .iter()
        .map(|emb| Code::<1>::quantize(emb, &centroid).as_ref().to_vec())
        .collect();
    let codes_4: Vec<Vec<u8>> = embeddings
        .iter()
        .map(|emb| Code::<4>::quantize(emb, &centroid).as_ref().to_vec())
        .collect();
    ...
    for _ in 0..N_QUERIES {
        ...
        let cq1_bytes = Code::<1>::quantize(&query, &centroid).as_ref().to_vec();
        let cq4_bytes = Code::<4>::quantize(&query, &centroid).as_ref().to_vec();
        let cq1 = Code::<1, _>::new(cq1_bytes.as_slice());
        let cq4 = Code::<4, _>::new(cq4_bytes.as_slice());
        ...
        for i in 0..N {
            ...
            let c1 = Code::<1, _>::new(codes_1[i].as_slice());
            let c4 = Code::<4, _>::new(codes_4[i].as_slice());

            let d4 = c4.distance_query(&df, &r_q, cn, cdq, qn);
            let d1f = c1.distance_query(&df, &r_q, cn, cdq, qn);
            let db = c1.distance_quantized_query(&df, &qq);
            // distance_code: both vectors quantized; error comes from both sides.
            let dc4 = c4.distance_code(&cq4, &df, cn, DIM);
            let dc1 = c1.distance_code(&cq1, &df, cn, DIM);
