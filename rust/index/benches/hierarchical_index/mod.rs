...
fn build_tree_node(
    ...
    if expand_this_level {
        ...
        // -----------------------------------------------------------------
        let threshold_factor = ((1.0 + cfg.expansion_factor) * (1.0 + cfg.expansion_factor)) as f32;
...
pub fn tree_node_size(node: &CentroidTreeNode) -> usize {
    match node {
        ...
        CentroidTreeNode::Internal { children, .. } => children.iter().map(tree_node_size).sum(),
    }
}
...
impl HierarchicalCentroidIndex {
    ...
    pub fn search_with_rerank(
        ...
        self.search_with_rerank_timed(query, k, rerank_factor, beam, tau_override)
            .0
    }
    ...
    fn search_quantized(
        ...
        loop {
            ...
            for node in &current {
                match node {
                    CentroidTreeNode::Leaf { keys, codes, .. } => {
                        let codes_buf = codes.as_ref().unwrap();
                        for (i, &key) in keys.iter().enumerate() {
                            let code_slice = &codes_buf[i * cs..(i + 1) * cs];
                            let code = Code::<1, &[u8]>::new(code_slice);
    ...
    pub fn diagnose_level_recall(
        ...
        loop {
            ...
            let r10 = gt_10.intersection(&reachable).count() as f64 / gt_10.len().max(1) as f64;
