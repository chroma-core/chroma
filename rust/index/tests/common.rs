...
/// Build a SparsePostingBlock from sorted (offset, value) pairs.
pub fn make_block(entries: &[(u32, f32)]) -> SparsePostingBlock {
    SparsePostingBlock::from_sorted_entries(entries)
}

/// Assert that two f32 values are within `tolerance` of each other.
...
/// Generate sequential entries: offsets [start, start+step, start+2*step, ...) with weight `weight`.
...
pub async fn get_all_entries(reader: &BlockSparseReader<'_>, dim: u32) -> Vec<(u32, f32)> {
    ...
    blocks
        .into_iter()
        .flat_map(|b| b.offsets().to_vec().into_iter().zip(b.values().to_vec().into_iter()))
        .collect()
}
