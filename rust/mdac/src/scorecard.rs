...
impl<'a> Scorecard<'a> {
    ...
    pub fn track(&self, tags: &[&str]) -> Option<Ticket> {
...
impl Drop for Ticket<'_, '_> {
    fn drop(&mut self) {
        self.scorecard.untrack(std::mem::take(&mut self.keys));
    }
}

impl Ticket<'_, '_> {
    pub fn untrack(self) {
        // rely upon drop method
    }
}
