...
//!
//! Search returns chunk-level hits; this route reassembles a whole page the way
//! the foundation-ui `readPageFromCollection` does: fetch every chunk for the
//! slug, order them by `chunk_id`, and join their documents. The per-page
//! metadata (title, categories, …) is stamped identically on every chunk, so it
//! is read off the head chunk. Like the other wiki routes it proxies to the FE
//! through [`WikiClient`](crate::wiki::WikiClient), which enforces auth, quota,
...
/// tool and `POST /api/read-page`. Modelled on the foundation-ui `toFullPage`
/// shape, minus fields the agent does not need: `url` (the API has no team slug
/// for it), `outline` (anchor ids are generated by the UI's markdown pipeline),
/// and `version`.
...
/// returning `None` when no such page exists.
pub(crate) async fn run_read_page(
    ...
    read_full_page(&collection, slug).await
}
...
mod tests {
    ...
    fn assemble_page_orders_chunks_and_joins_content() {
        ...
        // Joined with no separator, mirroring foundation-ui `joinWikiChunks`.
