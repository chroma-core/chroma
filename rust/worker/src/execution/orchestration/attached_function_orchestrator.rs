...
use super::compact::{CollectionCompactInfo, CompactWriters, CompactionContext, CompactionMetrics};
...
impl ChromaError for AttachedFunctionOrchestratorError {
    fn code(&self) -> ErrorCodes {
        match self {
            ...
            AttachedFunctionOrchestratorError::RecvError(e) => ErrorCodes::Internal,
...
impl AttachedFunctionOrchestrator {
    pub fn new(
        ...
        let orchestrator = AttachedFunctionOrchestrator {
    ...
    async fn finish_no_attached_function(&mut self, ctx: &ComponentContext<Self>) {
        let collection_info = match self.get_input_collection_info() {
            Ok(info) => info,
            Err(e) => {
    ...
    async fn finish_success(
        ...
        // Get the run nonce from the attached function
        let attached_function_run_nonce = NonceUuid::new();
    ...
    /// Convert ExecuteAttachedFunctionOutput to MaterializeLogOutput for the ApplyDataOrchestrator
    fn convert_materialized_output_to_log_records(
        &self,
        output: &ExecuteAttachedFunctionOutput,
    ) -> Vec<MaterializeLogOutput> {
        // TODO: Implement proper conversion from ExecuteAttachedFunctionOutput to MaterializeLogOutput
...
impl Orchestrator for AttachedFunctionOrchestrator {
    ...
    async fn initial_tasks(
        ...
        let collection_info = match self.get_input_collection_info() {
            Ok(info) => info,
            Err(e) => {
                // If we can't get collection info, we can't proceed
                self.terminate_with_result(Err(e), ctx).await;
                return vec![];
            }
        };
    ...
    for AttachedFunctionOrchestrator
{
    ...
    async fn handle(
        ...
        // self.output_context.output_collection_segments = Some(message.clone());
        ...
        if let Err(_) = self.set_output_collection_info(output_collection_info) {
            self.terminate_with_result(
                Err(AttachedFunctionOrchestratorError::InvariantViolation(
        ...
        let collection_info = self
            ...
            .expect("Input collection info should be set");
    ...
    for AttachedFunctionOrchestrator
{
    ...
    async fn handle(
        ...
        // Convert execution results to log records and finish
        // let log_records = self.convert_materialized_output_to_log_records(&message);
        // self.finish_success(log_records, ctx).await;
    }
}
