...
use chroma_types::{
    Chunk, Collection, CollectionUuid, InternalSchema, LogRecord, NonceUuid, SchemaError, Segment,
...
impl CompactOrchestrator {
    ...
    async fn do_task(
        ...
        let task_context = self.task_context.as_ref().unwrap();
        ...
        let output_collection = self
            .output_collection
            .get()
            .expect("Output collection should be set");
        let output_segments = self
            .output_segments
            .get()
            .expect("Output segments should be set");
        let output_record_segment = output_segments
            .iter()
            .find(|s| s.r#type == SegmentType::BlockfileRecord)
            .expect("Output record segment should exist");
        ...
        let execute_task_input = ExecuteTaskInput {
            ...
            output_collection_id: self.output_collection_id.get().unwrap().to_string(),
            completion_offset: task.completion_offset,
            output_record_segment: output_record_segment.clone(),
        ...
        vec![(
            wrap(
                Box::new(GetCollectionAndSegmentsOperator {
                    ...
                    output_collection_id: *self.output_collection_id.get().unwrap(),
                }),
                (),
                ctx.receiver(),
                self.context.task_cancellation_token.clone(),
...
impl Handler<TaskResult<PrepareTaskOutput, PrepareTaskError>> for CompactOrchestrator {
    ...
    async fn handle(
        ...
        let task_context = self.task_context.as_mut().unwrap();
    ...
    for CompactOrchestrator
{
    ...
    async fn handle(
        ...
        if self.task_context.is_some() {
            let task_completion_offset = <std::option::Option<chroma_types::Task> as Clone>::clone(
                &self.task_context.as_ref().unwrap().task,
            )
            .unwrap()
            .completion_offset;
            // completion_offset = u64::MAX (-1 in Go) means "no records processed yet"
            // Keep it as -1 so FetchLog fetches from offset 0
            // Otherwise, completion_offset is "last offset processed"
            // which matches the semantics of log_position
            input_collection.log_position = if task_completion_offset == u64::MAX {
                -1
            } else {
                task_completion_offset as i64
            };
        }
    ...
    for CompactOrchestrator
{
    ...
    async fn handle(
        ...
        if output.is_empty() && self.task_context.is_none() {
            ...
            self.pulled_log_offset = self.input_collection.get().unwrap().log_position;
            self.do_task(output, ctx).await;
        } else {
            self.partition(output, ctx).await;
        }
    ...
    async fn handle(
        ...
        self.terminate_with_result(
            Ok(CompactionResponse::Success {
                job_id: self.task_context.as_ref().unwrap().task_id.0,
            }),
            ctx,
        )
        .await;
...
impl Handler<TaskResult<RegisterOutput, RegisterError>> for CompactOrchestrator {
    ...
    async fn handle(
        ...
        if let Some(updated_task) = register_output.updated_task {
            ...
            self.terminate_with_result(
                Ok(CompactionResponse::Success {
                    job_id: self.output_collection_id.get().unwrap().0,
                }),
                ctx,
            )
            .await;
...
mod tests {
    ...
    async fn get_total_count_output(
        ...
        let reader =
            RecordSegmentReader::from_segment(&output_info.record_segment, blockfile_provider)
                .await
    ...
    async fn test_k8s_integration_task_execution() {
        ...
        // Step 1: Create input collection via HTTP API
        ...
        //     .json(&chroma_frontend::server::CreateCollectionPayload {
        ...
        //     .send()
        //     .await
        //     .expect("Should be able to create collection");
        ...
        // Step 3: Create a task via sysdb
        let task_id = sysdb
            .create_task(
                task_name.to_string(),
        ...
        let total_count = get_total_count_output(
            &mut sysdb,
        ...
        // Step 5: Add 50 more records and run again
        add_delete_generator
            .generate_vec(51..=100)
            .into_iter()
            .for_each(|log| {
        ...
        let total_count_2 = get_total_count_output(
            &mut sysdb,
            output_collection_id,
            &test_segments.blockfile_provider,
        )
