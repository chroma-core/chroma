    ...
    for LogFetchOrchestrator
{
    ...
    async fn handle(
        ...
        if self.context.is_rebuild {
            ...
            let scopes = &self.context.apply_segment_scopes;
            let should_clear = |scope: &SegmentScope| scopes.is_empty() || scopes.contains(scope);
            if should_clear(&SegmentScope::METADATA) {
                metadata_segment.file_path = Default::default();
            }
            if should_clear(&SegmentScope::RECORD) {
                record_segment.file_path = Default::default();
            }
            if should_clear(&SegmentScope::VECTOR) {
