...
use chroma_segment::types::{HydratedMaterializedLogRecord, MaterializeLogsResult};
use chroma_system::{Operator, OperatorType};
use chroma_types::{
    AttachedFunction, AttachedFunctionUuid, Chunk, CollectionUuid, LogRecord, Operation,
    OperationRecord, Segment, UpdateMetadataValue, FUNCTION_RECORD_COUNTER_ID,
...
impl AttachedFunctionExecutor for CountAttachedFunction {
    async fn execute(
        ...
        let output_record = LogRecord {
            log_offset: 0, // Will be set by the orchestrator
