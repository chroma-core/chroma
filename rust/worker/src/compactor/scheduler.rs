...
use crate::compactor::compaction_manager::JobMode;
...
pub(crate) struct Scheduler {
    ...
    in_progress_jobs: HashMap<JobUuid, InProgressJob>,
    job_expiry_seconds: u64,
    failing_jobs: HashMap<JobUuid, FailedJob>,
    dead_jobs: HashSet<JobUuid>,
...
impl Scheduler {
    ...
    pub(crate) fn get_dead_jobs(&self) -> Vec<JobUuid> {
    ...
    pub(crate) fn succeed_job(&mut self, job_uuid: JobUuid) {
        if self.in_progress_jobs.remove(&job_uuid).is_none() {
            tracing::warn!(
        ...
        self.failing_jobs.remove(&job_uuid);
    }

    pub(crate) fn fail_job(&mut self, job_uuid: JobUuid) {
        if self.in_progress_jobs.remove(&job_uuid).is_none() {
            tracing::warn!(
        ...
        match self.failing_jobs.get_mut(&job_uuid) {
            Some(failed_job) => {
            ...
            None => {
                self.failing_jobs.insert(job_uuid, FailedJob::new());
                tracing::warn!("Job for collection {} failed for the first time", job_uuid);
            }
        }
    }

    pub(crate) fn kill_job(&mut self, job_uuid: JobUuid) {
        self.failing_jobs.remove(&job_uuid);
        self.dead_jobs.insert(job_uuid);
