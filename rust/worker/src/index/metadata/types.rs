...
impl<'me> MetadataIndexReader<'me> {
    ...
    pub async fn lt(
        ...
        match self {
            MetadataIndexReader::U32MetadataIndexReader(blockfile_reader) => match key {
                KeyWrapper::Uint32(k) => {
                    let read = blockfile_reader.get_lt(prefix, *k).await;
                    match read {
                        Ok(records) => {
    ...
    pub async fn lte(
        ...
        match self {
            MetadataIndexReader::U32MetadataIndexReader(blockfile_reader) => match key {
                KeyWrapper::Uint32(k) => {
                    let read = blockfile_reader.get_lte(prefix, *k).await;
                    match read {
                        Ok(records) => {
                            let mut result = RoaringBitmap::new();
                            for (_, _, rbm) in records {
    ...
    pub async fn gt(
        ...
        match self {
            MetadataIndexReader::U32MetadataIndexReader(blockfile_reader) => match key {
                KeyWrapper::Uint32(k) => {
                    let read = blockfile_reader.get_gt(prefix, *k).await;
                    match read {
                        Ok(records) => {
                            let mut result = RoaringBitmap::new();
                            for (_, _, rbm) in records {
    ...
    pub async fn gte(
        ...
        match self {
            MetadataIndexReader::U32MetadataIndexReader(blockfile_reader) => match key {
                KeyWrapper::Uint32(k) => {
                    let read = blockfile_reader.get_gte(prefix, *k).await;
                    match read {
                        Ok(records) => {
                            let mut result = RoaringBitmap::new();
                            for (_, _, rbm) in records {
