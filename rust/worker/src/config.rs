...
impl RootConfig {
    ...
    pub(crate) fn load_from_path(path: &str) -> Self {
        ...
        let default_base_path = "./chroma_config_defaults.yaml";
        f = figment::Figment::from(Yaml::file(default_base_path));
        if std::path::Path::new(path).exists() {
            // f = figment::Figment::from(Yaml::file(path)).merge(f);
            f = f.merge(figment::Figment::from(Yaml::file(path)));
        }
...
mod tests {
    ...
    fn test_config_overriding() {
        Jail::expect_with(|jail| {
            let defaults = jail.create_file(
                "chroma_config_defaults.yaml",
                r#"
                query_service:
                    service_name: "query-service"
                    otel_endpoint: "http://jaeger:4317"
            ...
            let overrides_excluding_some = jail.create_file(
                "chroma_config.yaml",
                r#"
                query_service:
                    service_name: "query-service"
                    otel_endpoint: "http://jaeger:4317"
                    // DEMO: OVERRIDING SOME FIELDS
                    my_member_id: "query-service-10"
                    my_port: 50051
                    // DEMO: EXCLUDING SOME FIELDS
                    // assignment_policy:
                    //     RendezvousHashing:
                    //         hasher: Murmur3
                    // memberlist_provider:
                ...
                compaction_service:
                    ...
                    blockfile_provider:
                        Arrow:
                            block_manager_config:
                                // DEMO: CREATING A NEW SCHEMA IN AN EXISTING PATH
                                memory:
                                    chocolate: "yummy"
                                max_block_size_bytes: 16384
                                block_cache_config:
                    ...
                    hnsw_provider:
                        ...
                        hnsw_cache_config:
                            weighted_lru:
                                capacity: 1073741824
                    // DEMO: ADDING SOME FIELDS THAT ARE UNUSED
                    unused_field: "unused"
                "#,
            );
            let config = RootConfig::load();
            // DEMO: OVERRIDING SOME FIELDS
            assert_eq!(config.query_service.my_member_id, "query-service-10");
            Ok(())
        });
    }
}
