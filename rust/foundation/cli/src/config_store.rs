...
pub trait ConfigStore {
    ...
    fn get_current_profile(&self) -> Result<(String, Profile), FoundationError> {
        ...
        let profile = self.get_profile(&name).map_err(|_| FoundationError::NoActiveProfile)?;
        Ok((name, profile))
    }
}
...
impl ConfigStore for FileConfigStore {
    ...
    fn read_profiles(&self) -> Result<Profiles, FoundationError> {
        ...
        let contents = fs::read_to_string(path).map_err(|_| FoundationError::CredsFileReadFailed)?;
    ...
    fn write_profiles(&self, profiles: &Profiles) -> Result<(), FoundationError> {
        let path = self.credentials_path()?;
        let toml_str = toml::to_string(profiles).map_err(|_| FoundationError::CredsFileParseFailed)?;
    ...
    fn write_config(&self, config: &FoundationConfig) -> Result<(), FoundationError> {
        ...
        let json_str =
            serde_json::to_string_pretty(&value).map_err(|_| FoundationError::ConfigFileParseFailed)?;
