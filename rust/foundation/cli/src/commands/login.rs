...
fn select_team(teams: Vec<Team>, term: &mut dyn Terminal) -> Result<Team, FoundationError> {
    match teams.len() {
        ...
        _ => {
            ...
            term.println(&"Which team would you like to log in with?".blue().bold().to_string());
...
fn filter_team(team_id: &str, teams: Vec<Team>) -> Result<Team, FoundationError> {
    teams
        .into_iter()
        .find(|t| t.uuid == team_id)
        .ok_or_else(|| FoundationError::TeamNotFound(team_id.to_string()))
}
...
fn resolve_profile_name(
    ...
    match input.as_str() {
        "" => {
            term.println(&format!("{} {}\n", "Overriding profile".green(), default_name.green()));
            Ok(default_name)
        }
        _ => {
            term.println(&format!("{}\n", input.green()));
...
pub async fn run_login(
    ...
    profiles.insert(profile_name.clone(), Profile::new(api_key, team.uuid.clone()));
...
pub fn login(args: LoginArgs) -> i32 {
    ...
    match runtime.block_on(run_login(args, &client, &store, &mut term)) {
        Ok(()) => 0,
        Err(e) => {
...
pub fn logout(args: LogoutArgs) -> i32 {
    let store = FileConfigStore::default();
    let mut term = SystemTerminal;
    let runtime = match tokio::runtime::Runtime::new() {
        Ok(r) => r,
    ...
    match runtime.block_on(run_logout(args, &store, &mut term)) {
        Ok(()) => 0,
        Err(e) => {
            eprintln!("{} {}", "Error:".red().bold(), e);
            1
...
mod tests {
    ...
    fn test_filter_team_not_found() {
        ...
        assert!(matches!(err, FoundationError::TeamNotFound(_)));
    }
    ...
    fn test_login_sets_current_when_first_profile() {
        ...
        profiles.insert("new".to_string(), Profile::new("key".to_string(), "tenant".to_string()));
    ...
    async fn test_logout_removes_profile() {
        ...
        let mut term = TestTerminal::new();
        run_logout(LogoutArgs { profile: Some("my-team".to_string()) }, &store, &mut term)
            .await
            .unwrap();
    ...
    async fn test_logout_clears_current_profile() {
        let store = InMemoryConfigStore::new(make_profiles(&["my-team"]), make_config("my-team"));
        let mut term = TestTerminal::new();
        run_logout(LogoutArgs { profile: None }, &store, &mut term).await.unwrap();
    ...
    async fn test_logout_nonexistent_profile_errors() {
        let store = InMemoryConfigStore::new(make_profiles(&[]), make_config(""));
        let mut term = TestTerminal::new();
        let err = run_logout(LogoutArgs { profile: Some("ghost".to_string()) }, &store, &mut term)
            .await
            .unwrap_err();
