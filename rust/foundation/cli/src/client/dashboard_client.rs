...
impl DashboardClient {
    ...
    fn headers(&self, session_id: &str) -> Result<Option<HeaderMap>, FoundationError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&cookie_value)
                .map_err(|_| FoundationError::NetworkError("Failed to parse session cookie".to_string()))?,
        );
        Ok(Some(headers))
    }
    ...
    pub async fn verify_cli_token(&self, token: String) -> Result<CliVerifyResponse, FoundationError> {
